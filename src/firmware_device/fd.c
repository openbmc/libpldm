			    ...
			    size_t *resp_payload_len)
{
	...
	rc = fd->ops->imageset_versions(fd->ops_ctx, &active, &pending);
	if (rc) {
		return pldm_fd_reply_cc(PLDM_ERROR, hdr, resp,
					resp_payload_len);
	}
	...
	/* Add the fixed parameters */
	{
		...
		rc = encode_get_firmware_parameters_resp(
			hdr->instance, &fwp, &active, &pending, resp, &len);
		if (rc) {
				  ...
				  size_t *resp_payload_len, uint8_t address)
{
	...
	rc = encode_request_update_resp(hdr->instance, 0, 0, resp,
					resp_payload_len);
	if (rc) {
		return pldm_fd_reply_errno(rc, hdr, resp, resp_payload_len);
	}
			     ...
			     struct pldm_msg *resp, size_t *resp_payload_len)
{
	...
	rc = encode_pass_component_table_resp(hdr->instance, comp_resp,
			       ...
			       size_t *resp_payload_len)
{
	...
	rc = encode_update_component_resp(hdr->instance, comp_resp,
...
static int pldm_fd_cancel_update_comp(struct pldm_fd *fd,
				      const struct pldm_header_info *hdr,
				      const struct pldm_msg *req,
				 ...
				 size_t *resp_payload_len)
{
	...
	rc = encode_cancel_update_resp(hdr->instance, 0, zerobf, resp,
				       resp_payload_len);
	if (rc) {
		return pldm_fd_reply_errno(rc, hdr, resp, resp_payload_len);
	}
				     ...
				     size_t *resp_payload_len)
{
	...
	if (ccode == PLDM_SUCCESS ||
		...
		rc = encode_activate_firmware_resp(hdr->instance, ccode,
						   estimated_time, resp,
						   resp_payload_len);
		if (rc) {
			return pldm_fd_reply_errno(rc, hdr, resp,
...
static int pldm_fd_handle_transfer_complete_resp(struct pldm_fd *fd,
						 const struct pldm_msg *resp,
						 size_t resp_payload_len)
{
	if (fd->state != PLDM_FD_STATE_DOWNLOAD) {
...
static int pldm_fd_handle_verify_complete_resp(struct pldm_fd *fd,
					       const struct pldm_msg *resp,
					       size_t resp_payload_len)
{
...
static int pldm_fd_handle_apply_complete_resp(struct pldm_fd *fd,
					      const struct pldm_msg *resp,
					      size_t resp_payload_len)
{
		       ...
		       size_t *out_len)
{
	...
	size_t req_payload_len = in_len - sizeof(struct pldm_msg_hdr);
	...
	size_t resp_payload_len = *out_len - sizeof(struct pldm_msg_hdr);
		     ...
		     pldm_tid_t *address)
{
	...
	size_t req_payload_len = *out_len - sizeof(struct pldm_msg_hdr);
