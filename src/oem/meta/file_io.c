...
int encode_oem_meta_file_io_read_resp(
	...
	struct pldm_oem_meta_file_io_read_req *req_msg,
	struct pldm_msg *responseMsg, uint16_t payload_length)
{
	if (req_msg == NULL || responseMsg == NULL) {
		return -EINVAL;
	}
	...
	struct pldm_oem_meta_file_io_read_resp *response =
		(struct pldm_oem_meta_file_io_read_resp *)responseMsg->payload;
	response->completion_code = completion_code;
	response->handle = req_msg->handle;
	response->option = req_msg->option;
