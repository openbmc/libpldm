				     ...
				     uint16_t req_length)
{
	...
	rc = pldm_msgbuf_extract(buf, req->info.length);
	if (rc) {
		return rc;
	}
	...
	case PLDM_OEM_META_FILE_IO_READ_DATA:
		pldm_msgbuf_extract(buf, req->info.transferFlag);
		pldm_msgbuf_extract(buf, req->info.highOffset);
		pldm_msgbuf_extract(buf, req->info.lowOffset);
		break;
	default:
		return -EINVAL;
	}
