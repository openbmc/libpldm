				    ...
				    uint32_t last_record_handle)
{
	...
	for (record = repo->first; record; record = record->next) {
		...
		// TODO: Demonstrate how we know we have a pldm_pdr_header type. Why isn't the type
		//       specified that way in the struct definition?
		...
		// TODO: Define which way this relationship is meant to go. Are we trying to find
		//       some thing inside the range, or outside the range? Because the code was
		//       implemented as finding something outside the range (the original code
		//       negated the call to pldm_record_handle_in_range()).
		if (in_range) {
			continue;
		}
