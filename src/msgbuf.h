...
pldm_msgbuf_span_string_ascii(struct pldm_msgbuf *ctx, void **cursor)
{
	...
	ctx->remaining -= (intmax_t)strlen((char *)ctx->cursor) + 1;
	assert(ctx->remaining >= 0);
	if (ctx->remaining < 0) {
		return pldm_msgbuf_status(ctx, EOVERFLOW);
	}
...
pldm_msgbuf_span_string_utf16(struct pldm_msgbuf *ctx, void **cursor)
{
	...
	name = (char16_t *)ctx->cursor;
#if defined(__LITTLE_ENDIAN_BITFIELD)
	for (size_t j = 0; j < char16len(name); j++) {
		*(name + j) = be16toh(*(name + j));
	}
#endif

	*cursor = name;
	ctx->cursor += sizeof(char16_t) * (char16len(name) + 1);
