...
__attribute__((always_inline)) static inline int
pldm_msgbuf_extract_array_void(struct pldm_msgbuf *ctx, void *dst, size_t count)
{
	assert(ctx);
...
{
	return pldm_msgbuf_extract_array_void(ctx, dst, count);
}

__attribute__((always_inline)) static inline int
pldm_msgbuf_extract_array_uint8(struct pldm_msgbuf *ctx, void *dst,
				size_t count)
{
	return pldm_msgbuf_extract_array_void(ctx, dst, count);
}
...
__attribute__((always_inline)) static inline int
pldm_msgbuf_insert_array_void(struct pldm_msgbuf *ctx, const void *src,
			      size_t count)
{
	assert(ctx);
...
pldm_msgbuf_insert_array_char(struct pldm_msgbuf *ctx, const char *src,
			      size_t count)
{
	return pldm_msgbuf_insert_array_void(ctx, src, count);
}
