	...
	struct pldm_msg *resp, size_t *resp_payload_len)
{
	...
	if (!pd->ops.get_sensor_reading) {
		return pldm_platform_pd_reply_cc(
			PLDM_ERROR_UNSUPPORTED_PLDM_CMD, hdr, resp,
			resp_payload_len);
	}

	ccode = pd->ops.get_sensor_reading(pd->ops.ctx, sensor_id,
	...
	rc = encode_get_sensor_reading_resp(
		hdr->instance, PLDM_SUCCESS, state.sensor_data_size,
		     ...
		     const struct pldm_platform_pd_ops *ops, size_t ops_size)
{
	...
	if (pd) {
		if (pldm_platform_pd_setup(pd, sizeof(*pd), pdr, control, ops,
					   ops_size) == 0) {
			return pd;
		}
		free(pd);
...
LIBPLDM_ABI_TESTING
int pldm_platform_pd_setup(struct pldm_platform_pd *pd, size_t pd_size,
			   const pldm_pdr *pdr, struct pldm_control *control,
			   const struct pldm_platform_pd_ops *ops,
			   size_t ops_size)
{
	int rc;

	if (pd == NULL || pdr == NULL || ops == NULL || ops_size == 0) {
		return -EINVAL;
	}

	if (pd_size < sizeof(struct pldm_platform_pd)) {
	...
	if (ops_size > sizeof(pd->ops)) {
		const uint8_t *tail = (const uint8_t *)ops + sizeof(pd->ops);
		size_t tail_size = ops_size - sizeof(pd->ops);
		for (size_t i = 0; i < tail_size; i++) {
			if (tail[i] != 0) {
				return -E2BIG;
			}
		}
		memcpy(&pd->ops, ops, sizeof(pd->ops));
	} else {
		memcpy(&pd->ops, ops, ops_size);
	}
