	...
	struct variable_field *downstream_devices)
{
	...
	pldm_msgbuf_extract_p(buf, number_of_downstream_devices);
	...
	struct variable_field *downstream_device_param_table)
{
	...
	if (payload_length < PLDM_GET_DOWNSTREAM_FIRMWARE_PARAMS_RESP_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
	...
	rc = pldm_msgbuf_extract(buf, resp_data->downstream_device_count);
	if (rc) {
		return rc;
	}
	...
	const uint8_t *curr =
		msg->payload + PLDM_GET_DOWNSTREAM_FIRMWARE_PARAMS_RESP_BYTES;
	for (uint16_t i = 0; i < resp_data->downstream_device_count; i++) {
		struct pldm_component_parameter_entry *entry =
	...
	rc = pldm_msgbuf_span_required(
		buf, downstream_param_table_length,
