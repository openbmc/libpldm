	...
	struct pldm_query_downstream_devices_resp *resp_data)
{
	...
	rc = pldm_msgbuf_extract(buf, downstream_device_update_supported);
	if (rc) {
		return rc;
	}
	...
	resp_data->number_of_downstream_devices =
		le16toh(number_of_downstream_devices);

	pldm_msgbuf_extract(buf, max_number_of_downstream_devices);
	resp_data->max_number_of_downstream_devices =
		le16toh(max_number_of_downstream_devices);

	pldm_msgbuf_extract(buf, capabilities.value);
	resp_data->capabilities.value = le32toh(capabilities.value);
