	...
	struct variable_field *downstream_devices)
{
	...
	rc = pldm_msgbuf_extract_p(buf, next_data_transfer_handle);
	if (rc) {
		return rc;
	}
	...
	rc = pldm_msgbuf_extract_p(buf, downstream_devices_length);
	if (rc) {
		return rc;
	}
	rc = pldm_msgbuf_extract_p(buf, number_of_downstream_devices);
	if (rc) {
		return rc;
	}
	rc = pldm_msgbuf_span_required(buf, *downstream_devices_length,
