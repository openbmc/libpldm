			...
			size_t resp_size)
{
	...
	case PLDM_GET_PLDM_VERSION: {
		...
		int rc = decode_get_version_resp(
		    ...
		    resp_size - sizeof(struct pldm_msg_hdr), &completionCode,
		    &retTransferHandle, &retFlag, &versionOut);
	...
	case PLDM_GET_PLDM_COMMANDS: {
		// TODO: Add support to get PLDM_COMMANDS for available PLDM_TYPES
		uint8_t completionCode;
...
pldm_base_get_current_status(struct requester_base_context *ctx,
			     uint8_t *pldmType, status_t *status)
{
	*pldmType = ctx->next_command;
