...
pldm_transport_mctp_demux_core(struct pldm_transport_mctp_demux *t)
{
	return &t->transport;
}

static pldm_requester_rc_t pldm_transport_mctp_demux_open()
{
	int fd = -1;
	int rc = -1;
...
struct pldm_transport_mctp_demux *pldm_transport_mctp_demux_init()
{
...
int pldm_transport_mctp_demux_get_tid_by_eid(
    struct pldm_transport_mctp_demux *ctx, pldm_tid_t *tid, mctp_eid_t eid)
{
	int i;
	for (i = 0; i < ctx->num_mappings; i++) {
		if (ctx->tid_eid_map[i].eid == eid) {
			*tid = ctx->tid_eid_map[i].tid;
			return 0;
		}
	}
	return -1;
