...
pldm_requester_rc_t pldm_recv_msg_any_inst(struct pldm_transport *b,
					   mctp_eid_t eid,
					   uint8_t **pldm_resp_msg,
					   size_t *resp_msg_len)
{
	pldm_requester_rc_t rc = b->recv(b, eid, pldm_resp_msg, resp_msg_len);
...
pldm_requester_rc_t pldm_recv_msg(struct pldm_transport *b, mctp_eid_t eid,
...
/* don't destroy backend as we want the socket to stay open, as some apps use
 * the fd to it */
				  ...
				  uint8_t **pldm_resp_msg, size_t *resp_msg_len)
{
	...
	rc = pldm_recv_msg_any_inst(pldm_transport_demux_core(demux), eid,
			      ...
			      uint8_t **pldm_resp_msg, size_t *resp_msg_len)
{
	...
	rc = pldm_recv_msg(pldm_transport_demux_core(demux), eid, instance_id,
			   pldm_resp_msg, resp_msg_len);
				   ...
				   size_t *resp_msg_len)
{
	...
	rc = pldm_send_recv_msg(pldm_transport_demux_core(demux), eid,
				pldm_req_msg, req_msg_len, pldm_resp_msg,
				resp_msg_len);
			      ...
			      const uint8_t *pldm_req_msg, size_t req_msg_len)
{
	...
	rc = pldm_send_msg(pldm_transport_demux_core(demux), eid, pldm_req_msg,
			   req_msg_len);
