...
int pldm_transport_mctp_query_tid(struct pldm_transport_mctp_demux *ctx,
				  mctp_eid_t eid)
{
	// TODO - do PLDM GetTID cmd
	if (ctx && eid)
		return 0;
	return 0;
}

static int pldm_transport_mctp_find_tid(struct pldm_transport_mctp_demux *ctx,
				...
				pldm_tid_t tid, mctp_eid_t eid)
{
	...
	if (pldm_transport_mctp_find_tid(ctx, tid, eid) != -1) {
		return 0;
	}
...
int pldm_transport_mctp_unmap_tid(struct pldm_transport_mctp_demux *ctx,
				  pldm_tid_t tid, mctp_eid_t eid)
{
	int index = pldm_transport_mctp_find_tid(ctx, tid, eid);
	if (index == -1) {
		return 0;
	}
...
static int pldm_transport_mctp_get_eid(struct pldm_transport *t, pldm_tid_t tid,
				       mctp_eid_t *eid)
{
	// TODO read TID to EID mapping
	if (t && tid && eid)
		*eid = 99;
	return 0;
}
						     ...
						     size_t *resp_msg_len)
{
	...
	mctp_eid_t eid = 0;
	int rc = pldm_transport_mctp_get_eid(t, tid, &eid);
	if (rc) {
			  ...
			  const uint8_t *pldm_req_msg, size_t req_msg_len)
{
	struct pldm_transport_mctp_demux *demux = transport_to_demux(t);
	mctp_eid_t eid = 0;
	if (pldm_transport_mctp_get_eid(t, tid, &eid)) {
