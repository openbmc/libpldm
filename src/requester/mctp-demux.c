...
struct pldm_transport_mctp_demux {
	...
	int socket;
	int num_mappings;
	struct tid_eid_mapping *tid_eid_mapping;
};
				       ...
				       int *index)
{
	...
	for (i = 0; i < ctx->num_mappings; i++) {
		if (ctx->tid_eid_mapping[i].tid == tid &&
		    ctx->tid_eid_mapping[i].eid == eid) {
			*index = i;
			return 0;
		}
	}
				...
				pldm_tid_t tid, mctp_eid_t eid)
{
	...
	ctx->num_mappings++;
	ctx->tid_eid_mapping =
	    realloc(ctx->tid_eid_mapping,
		    ctx->num_mappings * sizeof(struct tid_eid_mapping));
	index = (ctx->num_mappings - 1);
	ctx->tid_eid_mapping[index].tid = tid;
	ctx->tid_eid_mapping[index].eid = eid;
				  ...
				  pldm_tid_t tid, mctp_eid_t eid)
{
	...
	for (i = index; i < ctx->num_mappings - 1; i++) {
		ctx->tid_eid_mapping[i].tid = ctx->tid_eid_mapping[i + 1].tid;
		ctx->tid_eid_mapping[i].eid = ctx->tid_eid_mapping[i + 1].eid;
	}
	ctx->num_mappings--;
	/* Zero the last mapping */
	ctx->tid_eid_mapping[ctx->num_mappings].tid = 0;
	ctx->tid_eid_mapping[ctx->num_mappings].eid = 0;
...
{
	if (ctx->tid_eid_mapping)
		free(ctx->tid_eid_mapping);
	free(ctx);
	ctx = NULL;
}
					 ...
					 pldm_tid_t *tid, mctp_eid_t eid)
{
	int i;
	for (i = 0; i < ctx->num_mappings; i++) {
		if (ctx->tid_eid_mapping[i].eid == eid) {
			*tid = ctx->tid_eid_mapping[i].tid;
			return 0;
		}
	}
	return -1;
