...
pldm_transport_afmctp_send(struct pldm_transport *b, mctp_eid_t eid,
...
{
	struct transport_afmctp *afmctp =
	    malloc(sizeof(struct transport_afmctp));
	if (!afmctp)
		return NULL;
...
void pldm_transport_afmctp_destroy(struct transport_afmctp *b) { free(b); }
