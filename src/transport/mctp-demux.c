			       ...
			       const void *pldm_req_msg, size_t req_msg_len)
{
	...
	if (demux->socket_send_buf.size >= 0 &&
	    (size_t)demux->socket_send_buf.size < req_msg_len) {
		if (pldm_socket_sndbuf_set(&(demux->socket_send_buf),
					   demux->socket, (int)req_msg_len)) {
			return PLDM_REQUESTER_SEND_FAIL;
		}
	}
...
int pldm_transport_mctp_demux_init(struct pldm_transport_mctp_demux **ctx)
{
	...
	if (pldm_socket_sndbuf_init(&(demux->socket_send_buf))) {
		free(demux);
		return -1;
	}

	if (pldm_socket_sndbuf_get(&demux->socket_send_buf, demux->socket)) {
		free(demux);
		return -1;
	}
