...
struct pldm_transport_af_mctp {
	...
	int socket;
	struct device_id tid_to_device_id[PLDM_MAX_TIDS];
...
pldm_transport_af_mctp_get_device_id(struct pldm_transport_af_mctp *ctx,
				     ...
				     uint32_t *network)
{
	if (ctx->tid_to_device_id[tid].eid == 0) {
		return -1;
	}
	*eid = ctx->tid_to_device_id[tid].eid;
	*network = ctx->tid_to_device_id[tid].network;
	return 0;
}
...
{
	for (int i = 0; i < 256; i++) {
		if (ctx->tid_to_device_id[i].eid == eid &&
		    ctx->tid_to_device_id[i].network == network) {
			*tid = i;
			return 0;
		}
	}
...
int pldm_transport_af_mctp_map_tid(struct pldm_transport_af_mctp *ctx,
				   ...
				   uint32_t network)
{
	ctx->tid_to_device_id[tid].eid = eid;
	ctx->tid_to_device_id[tid].network = network;
	return 0;
}
...
int pldm_transport_af_mctp_unmap_tid(struct pldm_transport_af_mctp *ctx,
				     pldm_tid_t tid)
{
	ctx->tid_to_device_id[tid].eid = 0;
	ctx->tid_to_device_id[tid].network = 0;
	return 0;
}
