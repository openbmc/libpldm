						       ...
						       size_t *msg_len)
{
	...
	if (af_mctp->is_responder && hdr->request) {
...
{
	struct pldm_transport_af_mctp *af_mctp = transport_to_af_mctp(t);
	const struct pldm_msg_hdr *hdr = pldm_msg;
	struct sockaddr_mctp addr = { 0 };
	...
	if (af_mctp->is_responder && !hdr->request) {
		...
		req = pldm_responder_cookie_untrack(&af_mctp->cookie_jar, 
				tid, hdr->instance_id, hdr->type, hdr->command);
		if (!req) {
...
{
	struct sockaddr_mctp lsmctp;
	int rc;

	if (!transport || !responder) {
	...
	struct pldm_responder_af_mctp *lresponder = malloc(sizeof(*responder));
	if (!lresponder) {
	...
	rc = bind(transport->socket, (struct sockaddr *)smctp, sizeof(*smctp));
	if (rc) {
		free(responder);
		return PLDM_REQUESTER_SETUP_FAIL;
	}

	transport->is_responder = true;
...
{
	struct pldm_transport_af_mctp *transport = responder->transport;
	free(responder);
	pldm_transport_af_mctp_destroy(transport);
}
