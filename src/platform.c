    ...
    uint8_t event_class_count)
{
	...
	*synchrony_config = response->synchrony_configuration;
	if ((*synchrony_config != PLDM_MESSAGE_TYPE_NOT_CONFIGURED) &&
	    (*synchrony_config != PLDM_MESSAGE_TYPE_ASYNCHRONOUS) &&
	    (*synchrony_config != PLDM_MESSAGE_TYPE_SYNCHRONOUS) &&
	    (*synchrony_config !=
	     PLDM_MESSAGE_TYPE_ASYNCHRONOUS_WITH_HEARTBEAT)) {
		return PLDM_ERROR_INVALID_DATA;
	}
