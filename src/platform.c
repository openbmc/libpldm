					 ...
					 set_effecter_state_field *field)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf,
		       ...
		       uint16_t *record_chg_num)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_GET_PDR_REQ_BYTES, msg->payload,
				      payload_length);
	if (rc) {
		return rc;
	}
			...
			uint8_t *transfer_crc)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_GET_PDR_MIN_RESP_BYTES,
				      msg->payload, payload_length);
	if (rc) {
		return rc;
	}
					...
					uint8_t event_class_count)
{
	...
	rc = pldm_msgbuf_init_cmdcode(
		buf, PLDM_EVENT_MESSAGE_SUPPORTED_MIN_RESP_BYTES, msg->payload,
		payload_length);
	if (rc) {
		return rc;
	}
			     ...
			     size_t *event_class_data_offset)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_SENSOR_EVENT_DATA_MIN_LENGTH,
					...
					uint32_t *data_transfer_handle)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_MSG_POLL_EVENT_LENGTH,
				      event_data, event_data_length);
	if (rc) {
		return rc;
	}
				  ...
				  uint8_t *rearm_event_state)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_GET_SENSOR_READING_REQ_BYTES,
				      msg->payload, payload_length);
	if (rc) {
		return rc;
	}
				   ...
				   uint8_t *completion_code)
{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_SET_EVENT_RECEIVER_RESP_BYTES,
				      msg->payload, payload_length);
	if (rc) {
		return rc;
	}
...

{
	...
	rc = pldm_msgbuf_init_cmdcode(buf, PLDM_SET_EVENT_RECEIVER_REQ_BYTES,
				      msg->payload, payload_length);
	if (rc) {
		return rc;
	}
