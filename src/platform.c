    ...
    struct pldm_numeric_sensor_value_pdr *pdr_value)
{
	...
	rc = pldm_msgbuf_init(buf, PLDM_NUMERIC_SENSOR_VALUE_PDR_MIN_SIZE, pdr_data,
			      pdr_data_length);
	if (rc < 0) {
		return rc;
	}

	pldm_msgbuf_extract_value_pdr_hdr(buf, &pdr_value->hdr);
	rc = pldm_platform_pdr_hdr_validate(&pdr_value->hdr,
		PLDM_NUMERIC_SENSOR_VALUE_PDR_MIN_SIZE, pdr_data_length);
	if (rc < 0) {
		return rc;
	}
	...
	pldm_msgbuf_extract(buf, &pdr_value->sensor_data_size);
	...
	pldm_msgbuf_extract_sensor_data(buf, pdr_value->sensor_data_size, &pdr_value->hysteresis);
	pldm_msgbuf_extract(buf, &pdr_value->supported_thresholds.byte);
	pldm_msgbuf_extract(buf, &pdr_value->threshold_and_hysteresis_volatility.byte);
	pldm_msgbuf_extract(buf, &pdr_value->state_transition_interval);
	pldm_msgbuf_extract(buf, &pdr_value->update_interval);
	pldm_msgbuf_extract_sensor_data(buf, pdr_value->sensor_data_size, &pdr_value->max_readable);
	pldm_msgbuf_extract_sensor_data(buf, pdr_value->sensor_data_size, &pdr_value->min_readable);

	pldm_msgbuf_extract(buf, &pdr_value->range_field_format);
	if (pdr_value->range_field_format > PLDM_RANGE_FIELD_FORMAT_MAX) {
	    return -PLDM_ERROR_INVALID_DATA;
	}

	pldm_msgbuf_extract(buf, &pdr_value->range_field_support.byte);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->nominal_value);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->normal_max);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->normal_min);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->warning_high);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->warning_low);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->critical_high);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->critical_low);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->fatal_high);
	pldm_msgbuf_extract_range_field_format(buf, pdr_value->range_field_format, &pdr_value->fatal_low);
