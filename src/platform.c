    ...
    uint16_t *event_id_to_acknowledge)
{
	...
	rc = pldm_msgbuf_extract(buf, data_transfer_handle);
	if (rc) {
		return rc;
	}
    ...
    size_t payload_length)
{
	...
	rc = pldm_msgbuf_insert(buf, completion_code);
	if (rc) {
		return rc;
	}
	...
	rc = pldm_msgbuf_insert(buf, transfer_flag);
	if (rc) {
		return rc;
	}
	...
	if ((event_data_size > 0) && event_data) {
		rc = pldm_msgbuf_insert_array(buf, event_data, event_data_size);
		if (rc) {
			return rc;
		}
	}
	...
	if (transfer_flag == PLDM_END || transfer_flag == PLDM_START_AND_END) {
		rc = pldm_msgbuf_insert(buf, checksum);
		if (rc) {
			return rc;
		}
	}
