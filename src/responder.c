...
{
	return left->terminus_id == right->terminus_id &&
...
pldm_responder_cookie_untrack(struct pldm_responder_cookie *jar,
			      pldm_tid_t terminus_id,
			      pldm_instance_id_t instance_id,
			      uint8_t type, uint8_t command)
{
	const struct pldm_responder_cookie cookie = {
		terminus_id, instance_id, type, command, NULL
	};
	struct pldm_responder_cookie *current;
	struct pldm_responder_cookie *next;
