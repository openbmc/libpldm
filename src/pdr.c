...
/* Insert new record to the end of pldm_pdr repo
 */
	...
	pldm_pdr *repo, pldm_entity *entity, uint32_t pdr_record_handle)
{
	...
	rc = pldm_msgbuf_init(src, PDR_MIN_SIZE, record->data, record->size);
	if (rc) {
		return rc;
	}
					   ...
					   uint32_t *entity_record_handle)
{
	...
	*entity_record_handle = pdr_record_handle + 1;
	...
	rc = pldm_pdr_insert_record(repo, curr, prev, new_record);
	if (rc) {
		goto cleanup_new_record_data;
	}
