	...
	pldm_pdr *repo, pldm_entity *entity, uint32_t updated_record_handle)
{
	...
	rc = pldm_msgbuf_init(src, PDR_MIN_SIZE, record->data,
			      le32toh(record->size));
	if (rc) {
		return rc;
	}
	...
	if ((le32toh(record->size)) >
	    (uint32_t)(SIZE_MAX - sizeof(pldm_entity))) {
		rc = -EOVERFLOW;
	...
	new_record->size =
		htole32(le32toh(record->size) + sizeof(struct pldm_entity));
	...
	rc = pldm_msgbuf_init(dst, PDR_MIN_SIZE, new_record->data,
			      le32toh(new_record->size));
	if (rc) {
	...
	if (le32toh(repo->size) < le32toh(record->size)) {
		rc = -ERANGE;
		goto cleanup_new_record_data;
	}
	repo->size = htole32(le32toh(repo->size) - le32toh(record->size));
	if (le32toh(repo->size) + le32toh(new_record->size) == UINT32_MAX) {
		rc = -EOVERFLOW;
		goto cleanup_new_record_data;
	}
	repo->size = htole32(le32toh(repo->size) + le32toh(new_record->size));
					   ...
					   uint32_t *updated_record_handle)
{
	...
	// Adding a new pdr with *entity as a child to *parent.
	// The number of children in this case 1.
	// size of new PDR = size of struct pldm_pdr_hdr +
	// size of struct pldm_pdr_entity_association.
	...
	*updated_record_handle = pdr_record_handle + 1;
	new_record->record_handle = htole32(*updated_record_handle);
	new_record->size = htole32(new_pdr_size);
	...
	pldm_msgbuf_insert(dst, *updated_record_handle);
	// header version
