...
uint16_t pldm_extract_remote_container_id(const pldm_entity_node *entity)
{
	assert(entity != NULL);
...
pldm_pdr_record *pldm_pdr_find_last_local_record(const pldm_pdr *repo)
{
	...
	while (curr != NULL) {
		if ((curr->record_handle > 0x00000000) &&
		    (curr->record_handle < PLDM_PDR_RECORD_HANDLE_HOST_START)) {
...
void entity_association_tree_find_if_remote(pldm_entity_node *node,
					    pldm_entity *entity,
