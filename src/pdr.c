...
#define PDR_ENTITY_ASSICIATION_MIN_SIZE                                        \
					 ...
					 pldm_entity **entities)
{
	...
	if (pdr_len < PDR_ENTITY_ASSICIATION_MIN_SIZE) {
		return;
	}
	...
	pldm_pdr *repo, pldm_entity *entity, uint32_t pdr_record_handle)
{
	...
	rc = pldm_msgbuf_init(src, PDR_ENTITY_ASSICIATION_MIN_SIZE,
	...
	rc = pldm_msgbuf_init(dst, PDR_ENTITY_ASSICIATION_MIN_SIZE,
					   ...
					   uint32_t *entity_record_handle)
{
	...
	new_pdr_size = PDR_ENTITY_ASSICIATION_MIN_SIZE;
	pldm_pdr_record *new_record = malloc(sizeof(pldm_pdr_record));
	if (!new_record) {
		return -ENOMEM;
	}
