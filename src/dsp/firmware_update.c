	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	if (!req_data || !msg ||
			!payload_length || *payload_length != PLDM_DOWNSTREAM_DEVICE_UPDATE_REQUEST_BYTES ||
	...
	pldm_msgbuf_insert(buf, req_data->maximum_downstream_device_transfer_size);
	pldm_msgbuf_insert(buf, req_data->maximum_outstanding_transfer_requests);
	pldm_msgbuf_insert(buf, req_data->downstream_device_package_data_length);
	...
	struct pldm_request_downstream_device_update_req *req)
{
	...
	if (!msg || payload_length != PLDM_DOWNSTREAM_DEVICE_UPDATE_REQUEST_BYTES || !req) {
		return -EINVAL;
	}
	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	rc = encode_pldm_header_only_errno(PLDM_RESPONSE, instance_id, PLDM_FWUP,
				     PLDM_REQUEST_DOWNSTREAM_DEVICE_UPDATE,
				     msg);
	if (rc) {
		return rc;
	...
	pldm_msgbuf_insert(buf, resp_data->downstream_device_will_send_get_package_data);
	pldm_msgbuf_insert(buf, resp_data->get_package_data_maximum_transfer_size);
	...
	struct pldm_request_downstream_device_update_resp *resp_data)
{
	...
	if (!msg || payload_length != PLDM_DOWNSTREAM_DEVICE_UPDATE_RESPONSE_BYTES || !resp_data) {
		return -EINVAL;
	}
	...
	pldm_msgbuf_extract(buf, resp_data->downstream_device_will_send_get_package_data);
	pldm_msgbuf_extract(buf,
