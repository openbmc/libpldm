				      ...
				      pldm_package_header_information_pad *hdr)
{
	...
	if (pin->format.revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR04H) {
		pldm_msgbuf_extract(buf, package_payload_checksum);
			rc = pldm_msgbuf_span_remaining(buf, &package_payload_offset, &package_payload_size);
		if (rc) {
			return pldm_msgbuf_discard(buf, rc);
		}
	}else{
	...
	rc = pldm_edac_crc32_validate(package_header_checksum,
								  data,
								  package_header_payload_size);
	if (rc) {
#if 0
	...
	struct pldm_package_firmware_device_id_record *rec)
{
	...
	int rc;
	size_t record_descriptors_remain_length;
	...
	/* The total length reserved for 
	   `package_data` and `reference_manifest_data` */
	record_descriptors_remain_length = rec->firmware_device_package_data.length + 
									   rec->reference_manifest_data.length;
	
	pldm_msgbuf_span_until(buf, record_descriptors_remain_length,
	...
	struct pldm_package_downstream_device_id_record *rec)
{
	...
	/* The total length reserved for 
	   `package_data` and `reference_manifest_data` */
	record_descriptors_remain_length = rec->package_data.length + 
									   rec->reference_manifest_data.length;
