...
{
	if(!downstream_devices || !pldm_downstream_devices ||
	   !downstream_descriptors) {
	   return PLDM_ERROR_INVALID_DATA;
	}
	struct pldm_msgbuf _buf;
	struct pldm_msgbuf* buf = &_buf;
	int rc = pldm_msgbuf_init_errno(buf, downstream_devices->length,
									  downstream_devices->ptr,
									  downstream_devices->length);
	if(rc) {
		return rc;
	}
	for(uint16_t device_count = 0;
		device_count < number_of_downstream_devices; 
		device_count++)
	{
		rc = pldm_msgbuf_span_required(buf, PLDM_DOWNSTREAM_DEVICE_BYTES,
									   (void**)(pldm_downstream_devices + device_count));
		if(rc) {
			return rc;
		}
		size_t downstream_descriptor_count = 
			pldm_downstream_devices[device_count]->downstream_descriptor_count;
		//uint8_t *descriptor_ptr = buf->cursor;

		struct pldm_msgbuf size_peeker = *buf;
		size_t downstream_descriptors_length = 0;
		while(downstream_descriptor_count--)
		{
			struct pldm_descriptor_tlv* pldm_descriptor;
			rc = pldm_msgbuf_span_required(&size_peeker, PLDM_FWUP_DEVICE_DESCRIPTOR_MIN_LEN,
										   (void**)&pldm_descriptor);
			if(rc){
				return rc;
			}
			void* payload;
			rc = pldm_msgbuf_span_required(&size_peeker, pldm_descriptor->descriptor_length - 1,
										   &payload);
			if(rc){
				return rc;
			}
			downstream_descriptors_length += PLDM_FWUP_DEVICE_DESCRIPTOR_MIN_LEN + 
											 pldm_descriptor->descriptor_length - 1;
		}
		downstream_descriptors[device_count].length = downstream_descriptors_length;
		rc = pldm_msgbuf_span_required(buf, downstream_descriptors[device_count].length,
				                       (void**)&downstream_descriptors[device_count].ptr);
		if (rc) {
			return rc;
		}
	}
...
LIBPLDM_ABI_TESTING
int pldm_extract_descriptor_buf(
	const struct variable_field *descriptors,
	...
	struct variable_field *descriptor_datas)
{
	...
	rc = pldm_msgbuf_init_errno(&descriptors_buf, descriptors->length,
						descriptors->ptr, descriptors->length);
	if (rc) {
		return rc;
	}
	for(size_t descriptor_count = 0; descriptor_count < number_of_descriptors; descriptor_count++)
	{
		struct pldm_msgbuf size_peeker = descriptors_buf;
		struct pldm_descriptor_tlv* descriptor_header;
		rc = pldm_msgbuf_span_required(&size_peeker,
									   PLDM_FWUP_DEVICE_DESCRIPTOR_MIN_LEN,
									   (void**)&descriptor_header);
		if(rc) {
			return rc;
		}
		descriptor_datas[descriptor_count].length = PLDM_FWUP_DEVICE_DESCRIPTOR_MIN_LEN +
									descriptor_header->descriptor_length - 1;
		rc = pldm_msgbuf_span_required(&descriptors_buf,
									   descriptor_datas[descriptor_count].length,
									   (void**)&descriptor_datas[descriptor_count].ptr);
		if(rc) {
			return rc;
		}
	}
