	...
	pldm_package_header_information_pad *header)
{
	...
	if (header->package_header_format_revision > PLDM_FWUP_PACKAGE_HEADER_FORMAT_REVISION_1_3) {
							      ...
							      size_t length)
{
	...
	uint32_t storedHeaderChecksum =
		(uint32_t)bytes[headerSize - 8] |
		((uint32_t)bytes[headerSize - 7] << 8) |
		((uint32_t)bytes[headerSize - 6] << 16) |
		((uint32_t)bytes[headerSize - 5] << 24);
