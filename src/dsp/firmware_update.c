...
void pldm_fwup_free_package_data(pldm_fwup_package_data* pkg_data)
{
    ...
    if (pkg_data->comp_image_infos)
    {
        ...
        {
            free_component_image_info(&pkg_data->comp_image_infos[i]);
        }
                                    ...
                                    pldm_fwup_descriptors* out_descs)
{
    ...
    for (uint8_t i = 0; i < descriptor_count; i++)
    {
        ...
        int rc = decode_descriptor_type_length_value(ptr, remaining, &desc_type,
                                                     &desc_data);
        if (rc)
        {
            return rc;
        ...
        desc->is_vendor_defined = (desc_type == PLDM_FWUP_VENDOR_DEFINED);
        ...
        if (desc->is_vendor_defined)
        {
            ...
            rc = decode_vendor_defined_descriptor_value(
                desc_data.ptr, desc_data.length, &title_str_type, &title_str,
                &vendor_data);
            if (rc)
            {
                return rc;
            ...
            desc->descriptor_data = (uint8_t*)calloc(desc->descriptor_length,
                                                     1);
            ...
            {
                desc->vendor_title = (char*)calloc(title_str.length + 1, 1);
                if (!desc->vendor_title)
                {
                    return -ENOMEM;
                }
                memcpy(desc->vendor_title, title_str.ptr, title_str.length);
            }
        }
        else
        {
        ...
        size_t used = sizeof(struct pldm_descriptor_tlv) - 1 + desc_data.length;
        ptr += used;
        remaining -= used;
                            ...
                            pldm_fwup_package_data* pkg_data)
{
    ...
    {
        pkg_data->device_id_records = (pldm_fwup_device_id_record*)calloc(
            device_id_record_count, sizeof(pldm_fwup_device_id_record));
        if (!pkg_data->device_id_records)
        {
            return -ENOMEM;
        }
    ...
    {
        if ((*offset + sizeof(struct pldm_firmware_device_id_record)) > length)
        {
            return -EINVAL;
        }
        ...
        int rc = decode_firmware_device_id_record(
            data + *offset, length - *offset, hdr->component_bitmap_bit_length,
            &tmp_rec, &applicable_components, &comp_img_set_version_str,
        ...
        pldm_fwup_device_id_record* rec = &pkg_data->device_id_records[i];
        ...
        if (applicable_components.length > 0)
        {
            ...
            rec->applicable_components = (size_t*)calloc(bit_count,
                                                         sizeof(size_t));
            ...
            for (size_t byte_idx = 0; byte_idx < applicable_components.length;
                 byte_idx++)
            {
                uint8_t bits = applicable_components.ptr[byte_idx];
                for (size_t bit = 0; bit < 8; bit++)
                {
                    if (bits & (1 << bit))
                    {
                        rec->applicable_components
                            [rec->num_applicable_components] = (byte_idx * 8) +
                                                               bit;
        ...
        {
            rec->comp_image_set_version =
                (char*)calloc(comp_img_set_version_str.length + 1, 1);
            if (!rec->comp_image_set_version)
            {
                return -ENOMEM;
            }
            memcpy(rec->comp_image_set_version, comp_img_set_version_str.ptr,
                   comp_img_set_version_str.length);
        }

        rc = parse_record_descriptors(&record_descs, tmp_rec.descriptor_count,
                                      &rec->descriptors);
        if (rc)
        {
            return rc;
                                  ...
                                  pldm_fwup_package_data* pkg_data)
{
    ...
    if (comp_count > 0)
    {
        pkg_data->comp_image_infos = (pldm_fwup_component_image_info*)calloc(
            comp_count, sizeof(pldm_fwup_component_image_info));
        if (!pkg_data->comp_image_infos)
        {
            return -ENOMEM;
        }
    ...
    {
        if ((*offset + sizeof(struct pldm_component_image_information)) >
            length)
        {
            return -EINVAL;
        }
        ...
        int rc = decode_pldm_comp_image_info(data + *offset, length - *offset,
                                             &tmp_info, &comp_version_field);
        if (rc)
        {
            return rc;
        }

        pldm_fwup_component_image_info* cinfo = &pkg_data->comp_image_infos[i];
        ...
        {
            cinfo->comp_version = (char*)calloc(comp_version_field.length + 1,
                                                1);
        ...
        size_t comp_info_len = sizeof(struct pldm_component_image_information) +
