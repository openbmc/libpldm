				      ...
				      pldm_package_header_information_pad *hdr)
{
	...
	if (pin->format.revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR04H) {
		...
		rc = pldm_msgbuf_span_remaining(buf, &package_data_offset,
						&package_payload_size);
		if (rc) {
			return pldm_msgbuf_discard(buf, rc);
		}
	} else {
		package_data_offset = NULL;
	...
	if (pin->format.revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR04H) {
		...
		if (rc) {
#if 0
				printf("payload checksum failure, expected: %#08" PRIx32 ", found: %#08" PRIx32 "\n", package_payload_checksum, pldm_edac_crc32(package_data_offset, package_payload_size));
#endif
			return rc;
		}
	}
