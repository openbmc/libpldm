	...
	pldm_package_header_information_pad *header)
{
	...
	if (header->package_header_format_revision >
	    PLDM_FWUP_PACKAGE_HEADER_FORMAT_REVISION_1_3) {
	...
	// Update checksums count for format revision 4+
	if (header->package_header_format_revision >=
	    PLDM_FWUP_PACKAGE_HEADER_FORMAT_REVISION_1_3) {
		checksums = 2;
	}
	...
	// Extract package payload checksum if format revision >= 4
...
#if 0
		printf("header checksum failed checksum failure, expected: %#08" PRIx32 ", found: %#08" PRIx32 "\n", package_header_checksum, pldm_edac_crc32(data, package_header_payload_size));
#endif
		return -EUCLEAN;
	}

	/* We stash these to resolve component images later */
	header->package.ptr = data;
	header->package.length = length;
	...
	const void *data, size_t length, struct pldm_payload_checksum_ctx *ctx)
{
	if (!data || length == 0 || !ctx) {
		fprintf(stderr, "Invalid input: data=%p, length=%zu, ctx=%p\n",
			data, length, (void *)ctx);
		return -EINVAL;
	}
	...
	struct pldm_payload_checksum_ctx *ctx)
{
	...
	if (ctx->bytes_processed >= ctx->payload_size) {
		...
		double elapsed =
			(double)(end.tv_sec - ctx->start_time.tv_sec) +
			(double)(end.tv_nsec - ctx->start_time.tv_nsec) /
				1000000000.0;
		...
		if (ctx->current_checksum != ctx->stored_checksum) {
			fprintf(stderr,
				...
				ctx->stored_checksum, ctx->current_checksum,
				elapsed, ctx->payload_size);
			return -EUCLEAN;
		}

		printf("Payload checksum match: expected 0x%08x, calculated 0x%08x (time: %f seconds, size: %zu bytes)\n",
		       ctx->stored_checksum, ctx->current_checksum, elapsed,
		       ctx->payload_size);
	...
	double progress = (double)ctx->bytes_processed /
			  (double)ctx->payload_size * 100.0;
	fprintf(stderr, "Checksum progress: %.1f%% (%zu/%zu bytes)\n", progress,
		ctx->bytes_processed, ctx->payload_size);
