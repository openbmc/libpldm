				      ...
				      pldm_package_header_information_pad *hdr)
{
	...
	int checksums = (pin->format.revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR04H) ? 2 : 1;
	int rc;
	...
	package_payload_size =length - package_header_size;

	if (pin->format.revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR04H) {
		if (package_payload_checksum !=
			pldm_edac_crc32((const uint8_t *)data + package_header_size, package_payload_size)) {
#if 0
			printf("checksum failure, expected: %#08" PRIx32 ", found: %#08" PRIx32 "\n", package_payload_checksum, pldm_edac_crc32((const uint8_t *)data + package_header_size,
	...
	struct pldm_package_firmware_device_id_record *rec)
{
	...
	if (hdr->package_header_format_revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR04H) {
	...
	if (!rec->reference_manifest_data.length) {
		rec->reference_manifest_data.ptr = NULL;
	}
	...
	struct pldm_package_component_image_information *info)
{
	...
	if (hdr->package_header_format_revision >= PLDM_PACKAGE_HEADER_FORMAT_REVISION_FR03H) {
		rc = pldm_msgbuf_extract_uint32_to_size(
