...
#define PLDM_FWUP_CHECKSUM_CHUNK_SIZE (8 * 1024) // 8K chunks

// Structure to hold the checksum calculation state
...
// Global context for checksum calculation
    ...
    const void *data, size_t length)
{
    ...
    // Ensure we have enough data for a basic header check
    ...
    // Check package header format revision
    const uint8_t formatRevision = bytes[16];
    fprintf(stderr, "Package header format revision: %u\n", formatRevision);
    
    // Payload checksum is only supported in format revision 3+
    if (formatRevision < 3) {
    ...
    // Ensure there's actually payload data (at least 1 byte after header)
    if (payloadSize == 0) {
        fprintf(stderr, "No payload data found\n");
        return -EBADMSG;
    }
    
    // Extract the stored payload checksum
...
int verify_pldm_firmware_update_package_payload_checksum_process(void)
{
    ...
    // If we've already processed everything, return completion status
    ...
    // For the first chunk, initialize the CRC
    if (g_checksum_ctx.bytes_processed == 0) {
        // Initialize CRC32 calculation
        g_checksum_ctx.current_checksum = 0xFFFFFFFF; // Initial value for CRC32
    }
    ...
    // If this is the last chunk, finalize the CRC
    if (g_checksum_ctx.bytes_processed >= g_checksum_ctx.payload_size) {
        g_checksum_ctx.current_checksum = ~g_checksum_ctx.current_checksum; // Final XOR
    }
    
    // Return 1 to indicate more processing is needed
    return 1;
}
