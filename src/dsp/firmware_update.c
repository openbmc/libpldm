	...
	pldm_package_header_information_api *header)
{
	...
	if (header->package_header_format_revision >= 3) {
		...
		if (header->package_payload_checksum !=
			...
			printf("Payload checksum mismatch: expected %#08x, calculated %#08x (time: %.6f seconds)\n",
		...
		if (header->package_payload_checksum ==
		    calculated_payload_checksum) {
			printf("Payload checksum match: expected %#08x, calculated %#08x (time: %.6f seconds, size: %zu bytes)\n",
			       header->package_payload_checksum,
			       calculated_payload_checksum,
			       elapsed_time,
			       payload_size);
