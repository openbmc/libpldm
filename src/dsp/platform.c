				   ...
				   struct pldm_msg *msg, size_t payload_length)
{
	...
	/* Calculate expected payload length based on sensor data size */
	switch (sensor_data_size) {
	case PLDM_SENSOR_DATA_SIZE_UINT8:
	case PLDM_SENSOR_DATA_SIZE_SINT8:
		expected_length = PLDM_GET_SENSOR_READING_MIN_RESP_BYTES;
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT16:
	case PLDM_SENSOR_DATA_SIZE_SINT16:
		expected_length = PLDM_GET_SENSOR_READING_MIN_RESP_BYTES + 1;
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT32:
	case PLDM_SENSOR_DATA_SIZE_SINT32:
		expected_length = PLDM_GET_SENSOR_READING_MIN_RESP_BYTES + 3;
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT64:
	case PLDM_SENSOR_DATA_SIZE_SINT64:
		expected_length = PLDM_GET_SENSOR_READING_MIN_RESP_BYTES + 7;
		break;
	default:
		return PLDM_ERROR_INVALID_DATA;
	}
	...
	case PLDM_SENSOR_DATA_SIZE_UINT8:
	case PLDM_SENSOR_DATA_SIZE_SINT8:
		rc = pldm_msgbuf_insert(buf, *present_reading);
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT16: {
		uint16_t val = *(uint16_t *)present_reading;
		rc = pldm_msgbuf_insert(buf, val);
		break;
	}
	case PLDM_SENSOR_DATA_SIZE_SINT16: {
		int16_t val = *(int16_t *)present_reading;
		rc = pldm_msgbuf_insert(buf, val);
		break;
	}
	case PLDM_SENSOR_DATA_SIZE_UINT32: {
	...
	case PLDM_SENSOR_DATA_SIZE_SINT32: {
		int32_t val = *(int32_t *)present_reading;
		rc = pldm_msgbuf_insert(buf, val);
		break;
	}
	case PLDM_SENSOR_DATA_SIZE_UINT64: {
