	...
	struct pldm_platform_effecter_auxiliary_names_iter *iter)
{
	...
	rc = pldm_msgbuf_init_errno(
		buf, PLDM_PLATFORM_EFFECTER_AUXILIARY_NAMES_PDR_MIN_LENGTH,
		data, data_length);
	if (rc) {
		return rc;
	}
	...
	rc = pldm_msgbuf_extract_value_pdr_hdr(
		buf, &pdr->hdr,
		PLDM_PLATFORM_EFFECTER_AUXILIARY_NAMES_PDR_MIN_LENGTH,
		data_length);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}
	...
	struct pldm_platform_effecter_auxiliary_name *effecter)
{
	...
	pldm_msgbuf_extract(buf, effecter->name_string_count);
	...
	struct pldm_platform_effecter_auxiliary_name_string *entry)
{
	...
	entry->tag.ptr = NULL;
	entry->tag.length = 0;
	pldm_msgbuf_span_string_ascii(buf, (const void **)&entry->tag.ptr,
				      &entry->tag.length);

	entry->name.ptr = NULL;
	entry->name.length = 0;
	pldm_msgbuf_span_string_utf16(buf, (const void **)&entry->name.ptr,
				      &entry->name.length);
