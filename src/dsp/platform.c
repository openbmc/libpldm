	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	for (uint8_t i = 0; i < req->composite_sensor_count; i++) {
		if (req->fields[i].sensor_operational_state >
		    PLDM_SET_SENSOR_UNAVAILABLE) {
			return -EINVAL;
		}

		if (req->fields[i].sensor_event_message_enable >
	...
	for (uint8_t i = 0; i < req->composite_sensor_count; i++) {
		pldm_msgbuf_insert(buf,
				   req->fields[i].sensor_operational_state);
		pldm_msgbuf_insert(buf,
	...
	struct pldm_platform_set_state_sensor_enables_req *req)
{
	...
	for (uint8_t i = 0; i < req->composite_sensor_count; i++) {
		...
		rc = pldm_msgbuf_extract(
			buf, req->fields[i].sensor_event_message_enable);
		if (rc) {
			return pldm_msgbuf_discard(buf, rc);
		}
