	...
	struct pldm_platform_numeric_sensor_pdr *pdr)
{
	...
	rc = pldm_msgbuf_extract_value_pdr_hdr(
		buf, &pdr->hdr, PLDM_PDR_NUMERIC_SENSOR_PDR_MIN_LENGTH,
		pdr_data_length);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}
	...
	pldm_msgbuf_extract(buf, pdr->base_oem_unit_handle);
	pldm_msgbuf_extract(buf, pdr->aux_unit);
	pldm_msgbuf_extract(buf, pdr->aux_unit_modifier);
	pldm_msgbuf_extract(buf, pdr->aux_rate_unit);
	pldm_msgbuf_extract(buf, pdr->rel);
	pldm_msgbuf_extract(buf, pdr->aux_oem_unit_handle);
	pldm_msgbuf_extract(buf, pdr->is_linear);

	rc = pldm_msgbuf_extract(buf, pdr->sensor_data_size);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}
	if (pdr->sensor_data_size > PLDM_SENSOR_DATA_SIZE_MAX) {
	...
	pldm_msgbuf_extract(buf, pdr->resolution);
	pldm_msgbuf_extract(buf, pdr->offset);
	pldm_msgbuf_extract(buf, pdr->accuracy);
	pldm_msgbuf_extract(buf, pdr->plus_tolerance);
	pldm_msgbuf_extract(buf, pdr->minus_tolerance);
	pldm_msgbuf_extract_sensor_data(buf, pdr->sensor_data_size,
					&pdr->hysteresis);
	pldm_msgbuf_extract(buf, pdr->supported_thresholds.byte);
	pldm_msgbuf_extract(buf, pdr->threshold_and_hysteresis_volatility.byte);
	pldm_msgbuf_extract(buf, pdr->state_transition_interval);
	pldm_msgbuf_extract(buf, pdr->update_interval);
	pldm_msgbuf_extract_sensor_data(buf, pdr->sensor_data_size,
					&pdr->max_readable);
	pldm_msgbuf_extract_sensor_data(buf, pdr->sensor_data_size,
					&pdr->min_readable);

	rc = pldm_msgbuf_extract(buf, pdr->range_field_format);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}
	if (pdr->range_field_format > PLDM_RANGE_FIELD_FORMAT_MAX) {
	...
	pldm_msgbuf_extract_range_field_format(buf, pdr->range_field_format,
					       pdr->nominal_value);
	pldm_msgbuf_extract_range_field_format(buf, pdr->range_field_format,
					       pdr->normal_max);
	pldm_msgbuf_extract_range_field_format(buf, pdr->range_field_format,
