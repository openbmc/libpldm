	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	sensor_operational_state = req->sensor_operational_state;
	sensor_event_message_enable = req->sensor_event_message_enable;
	...
	struct pldm_platform_set_numeric_sensor_enable_req *req)
{
	...
	if (sensor_operational_state > PLDM_SET_SENSOR_UNAVAILABLE) {
		return -EPROTO;
	}
	...
	req->sensor_operational_state = sensor_operational_state;
	req->sensor_event_message_enable = sensor_event_message_enable;
