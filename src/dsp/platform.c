...
{
	union_sensor_data_size64 reading;

	if (present_reading == NULL) {
		return PLDM_ERROR_INVALID_DATA;
	}

	int rc = decode_numeric_sensor_data_to_union(
		sensor_data, sensor_data_length, event_state,
		previous_event_state, sensor_data_size, &reading);
	...
	case PLDM_SENSOR_DATA_SIZE_UINT8:
		*present_reading = reading.value_u8;
		break;
	case PLDM_SENSOR_DATA_SIZE_SINT8:
		*present_reading = (uint32_t)(int32_t)reading.value_s8;
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT16:
		*present_reading = reading.value_u16;
		break;
	case PLDM_SENSOR_DATA_SIZE_SINT16:
		*present_reading = (uint32_t)(int32_t)reading.value_s16;
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT32:
		*present_reading = reading.value_u32;
		break;
	case PLDM_SENSOR_DATA_SIZE_SINT32:
		*present_reading = (uint32_t)reading.value_s32;
		break;
	default:
		return PLDM_ERROR_INVALID_DATA;
	}
...
LIBPLDM_ABI_TESTING
int decode_numeric_sensor_data_to_union(
	const uint8_t *sensor_data, size_t sensor_data_length,
	uint8_t *event_state, uint8_t *previous_event_state,
	uint8_t *sensor_data_size, union_sensor_data_size64 *present_reading)
{
	PLDM_MSGBUF_RO_DEFINE_P(buf);
	int rc;
	...
	rc = pldm_msgbuf_extract_sensor_data64(buf, *sensor_data_size,
					       present_reading);
	if (rc) {
		return pldm_xlate_errno(pldm_msgbuf_discard(buf, rc));
	}
