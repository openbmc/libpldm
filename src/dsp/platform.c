				    ...
				    size_t pdr_length)
{
	...
	rc = pldm_msgbuf_span_string_ascii(buf, (void **)&pdr->file_name.ptr,
					   &pdr->file_name.length);
	if (rc) {
		return rc;
	}
	if (pdr->file_name.length != pdr->file_name_len) {
		return -EOVERFLOW;
	}
	...
	if (pdr->oem_file_classification) {
		...
		rc = pldm_msgbuf_span_string_ascii(
			buf, (void **)&pdr->oem_file_classification_name.ptr,
			&pdr->oem_file_classification_name.length);
		if (rc) {
			return rc;
		}
		if (pdr->oem_file_classification_name.length !=
		    pdr->oem_file_classification_name_len) {
			return -EOVERFLOW;
		}
	}
