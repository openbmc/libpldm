	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	struct pldm_get_date_time_resp *response =
		(struct pldm_get_date_time_resp *)msg->payload;
	response->completion_code = resp->completion_code;
	response->seconds = resp->seconds;
	response->minutes = resp->minutes;
	response->hours = resp->hours;
	response->day = resp->day;
	response->month = resp->month;
