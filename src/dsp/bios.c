	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	pack_rc = pack_pldm_header(&header, &(msg->hdr));
	if (pack_rc != PLDM_SUCCESS) {
		return pack_rc;
	}
	...
	/* Use memcpy to safely read year from potentially misaligned packed struct */
	memcpy(&year, &resp->year, sizeof(year));
	...
	pldm_msgbuf_insert_uint16(buf, year);
