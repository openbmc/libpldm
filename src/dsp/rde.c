...
/* A varstring is well formed per DSP0218 Table 2: stringFormat names a defined
 * format, and stringData is NULL-terminated with a length that counts the
 * terminator and fits the uint8 stringLengthBytes wire field. */
static bool rde_varstring_valid(const struct pldm_rde_varstring *varstring)
{
	if (varstring->string_format >= PLDM_RDE_VARSTRING_MAX) {
		return false;
	}
	if (varstring->string_data.length == 0 ||
	    varstring->string_data.length > UINT8_MAX ||
	    varstring->string_data.ptr == NULL) {
		return false;
	}
	return varstring->string_data.ptr[varstring->string_data.length - 1] ==
	       '\0';
}
	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	if (!rde_varstring_valid(&resp->provider_name)) {
		return -EINVAL;
	}
	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	/* Validate before writing the header so -EINVAL leaves msg
	 * untouched, matching the request encoder. Error responses skip the
	 * check as they carry only the completion code.
	 */
	if (resp->completion_code == PLDM_SUCCESS &&
					...
					struct pldm_rde_varstring *varstring)
{
	...
	varstring->string_data.ptr = NULL;
	rc = pldm_msgbuf_span_required(
		buf, varstring->string_data.length,
	...
	if (!rde_varstring_valid(varstring)) {
		return -EPROTO;
	}
	return 0;
}
	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	if (!rde_varstring_valid(&resp->etag)) {
		return -EINVAL;
	}
				       ...
				       struct pldm_rde_operation_init_req *req)
{
	...
	rc = pldm_msgbuf_extract_uint32_to_size(buf,
						req->request_payload.length);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}
	...
	struct variable_field *response_payload)
{
	...
	/* The response always carries a NULL-terminated ETag varstring, so the
	 * floor budgets it beyond the fixed prefix. */
	rc = pldm_msgbuf_init_errno(buf, PLDM_RDE_OPERATION_INIT_RESP_MIN_BYTES,
				    msg->payload, payload_length);
	if (rc) {
		return rc;
	}
	...
	rc = pldm_msgbuf_extract_uint32_to_size(buf, response_payload->length);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}
	pldm_msgbuf_extract(buf, etag->string_format);
	rc = pldm_msgbuf_extract_uint8_to_size(buf, etag->string_data.length);
	if (rc) {
		return pldm_msgbuf_discard(buf, rc);
	}

	etag->string_data.ptr = NULL;
	rc = pldm_msgbuf_span_required(buf, etag->string_data.length,
	...
	rc = pldm_msgbuf_span_required(buf, response_payload->length,
				       ...
				       struct pldm_rde_multipart_send_req *req)
{
	...
	/* data.length receives the on-wire DataLengthBytes, which counts the
	 * data plus any trailing checksum; it is reduced to the data length
	 * once the checksum size is known. */
	...
	has_checksum = pldm_rde_transfer_flag_has_checksum(req->transfer_flag);
	checksum_size = has_checksum ? sizeof(req->data_integrity_checksum) : 0;
	if (req->data.length < checksum_size) {
		return pldm_msgbuf_discard(buf, -EBADMSG);
	}
	req->data.length -= checksum_size;

	req->data.ptr = NULL;
	rc = pldm_msgbuf_span_required(buf, req->data.length,
...
/* A receive request carries only these three operations per DSP0218 Table 72;
 * COMPLETE is a response-only value the enum also holds (Table 71), so the
 * accepted values are named rather than bounded. */
static bool rde_multipart_receive_operation_valid(uint8_t transfer_operation)
{
	switch (transfer_operation) {
	case PLDM_RDE_TRANSFER_OPERATION_FIRST_PART:
	case PLDM_RDE_TRANSFER_OPERATION_NEXT_PART:
	case PLDM_RDE_TRANSFER_OPERATION_ABORT:
		return true;
	default:
		return false;
	}
	...
	struct pldm_msg *msg, size_t *payload_length)
{
	...
	if (!rde_multipart_receive_operation_valid(req->transfer_operation)) {
		return -EINVAL;
	}
	...
	struct pldm_rde_multipart_receive_resp *resp)
{
	...
	checksum_size = has_checksum ? sizeof(resp->data_integrity_checksum) :
				       0;
	if (resp->data.length < checksum_size) {
		return pldm_msgbuf_discard(buf, -EBADMSG);
	}
	resp->data.length -= checksum_size;

	resp->data.ptr = NULL;
	rc = pldm_msgbuf_span_required(buf, resp->data.length,
