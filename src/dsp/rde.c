	...
	size_t *payload_length)
{
	...
	for (uint8_t i = 0; i < resp->string_fragment_count; i++) {
		/* Each fragment is NULL-terminated per DSP0218 Table 2, so its
		 * length includes at least the terminator and must fit the
		 * uint8 wire field. */
		if (uris[i].string_data.length == 0 ||
		    uris[i].string_data.length > UINT8_MAX ||
