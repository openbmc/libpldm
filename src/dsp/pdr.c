					  ...
					  uint32_t *record_handle)
{
	...
	while (record != NULL) {
		...
		uint8_t skip_data_size = sizeof(uint32_t) + sizeof(uint8_t);
		void *skip_data = NULL;
		uint8_t hdr_type = 0;
		pldm_msgbuf_span_required(buf, skip_data_size, &skip_data);
		...
		int match = pldm_pdr_record_matches_fru_rsi(record, fru_rsi);
		if (match) {
			*record_handle = record->record_handle;
