...
int encode_base_multipart_receive_req(
	...
	uint32_t transfer_ctx, uint32_t transfer_handle,
	uint32_t section_offset, uint32_t section_length, struct pldm_msg *msg,
	size_t payload_length)
{
	...
	pldm_msgbuf_insert(buf, section_offset);
	pldm_msgbuf_insert(buf, section_length);
				       ...
				       uint32_t *data_integrity_checksum)
{
	...
	rc = pldm_msgbuf_extract(buf, data_length);
	if (rc) {
		return rc;
	}
