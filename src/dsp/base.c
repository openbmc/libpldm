				  ...
				  uint32_t *data_integrity_checksum)
{
	if (msg == NULL || completion_code == NULL ||
	    transfer_opflag == NULL || transfer_handle == NULL ||
	    data_length == NULL || data_integrity_checksum == NULL) {
		return PLDM_ERROR_INVALID_DATA;
	}
...
//		memcpy(*data, (uint8_t *)response->data, *data_length);
		*data = response->data;
	}

	if (*transfer_opflag == PLDM_XFER_END) {
		*data_integrity_checksum = le32toh(response->data_integrity_checksum);
	} else {
		*data_integrity_checksum = 0;
	}
