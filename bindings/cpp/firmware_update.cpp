	...
	struct pldm_package_format_pin &pin) noexcept
{
						       ...
						       deviceIdRecordData, rc)
	{
		...
		fwDeviceIDRecords[fwDeviceIDRecords.size() - 1]
			.applicableComponents.reserve(componentsList.size());
	}
	...
	{
		// necessary to run this loop to prepare our iterator
		// for going over the component image.

		// TODO: expand support beyond v1.0.0 of package format at a later point
	}
	...
	foreach_pldm_package_component_image_information(packageItr, imageInfo,
							 rc)
	{
	...
	std::vector<FirmwareDeviceIDRecord> copyFwDeviceIDRecords = {};

	// resolve references to applicable components
	for (size_t i = 0; i < componentsListList.size(); i++) {
		std::vector<std::reference_wrapper<ComponentImageInfo> >
			applicableComponents;

		for (const auto ac : componentsListList[i]) {
			if (ac >= componentImageInfos.size()) {
				return std::unexpected(PackageParserError(std::format(
					"referencing nonexistent component image at index {}",
					ac)));
			}

			std::reference_wrapper<
				pldm::fw_update::ComponentImageInfo>
				refw = std::ref(componentImageInfos[ac]);
			applicableComponents.push_back(refw);
		}

		FirmwareDeviceIDRecord copyRecord = {
			...
			.descriptors = fwDeviceIDRecords[i].descriptors,
			.firmwareDevicePackageData =
				fwDeviceIDRecords[i].firmwareDevicePackageData,
		};

		copyFwDeviceIDRecords.push_back(copyRecord);
	}

	auto package = std::make_unique<Package>(copyFwDeviceIDRecords,
						 componentImageInfos);
