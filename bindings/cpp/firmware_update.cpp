				      ...
				      PackagePin pin) noexcept
{
	...
	{
		auto selfContainedActivationMinVersion = utils::toString(
			downstreamDeviceId
				.self_contained_activation_min_version_string_type,
			downstreamDeviceId
				.self_contained_activation_min_version_string);
		if (!selfContainedActivationMinVersion.has_value()) {
			return std::unexpected(PackageParserError(
				selfContainedActivationMinVersion.error()));
		}

		// TODO: spec (DSP0267, v1.3.0, Table 5)says this could also be a string or not exist at all. The libpldm struct does not seem to implement that possibility
		std::optional<std::variant<std::string, uint32_t> >
		...
		downstreamDeviceIDRecords.emplace_back(DownstreamDeviceIDRecord(
			...
			selfContainedActivationMinVersion.value(),
			selfContainedActivationMinVersionComparisonStamp,
