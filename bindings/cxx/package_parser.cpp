...
namespace fw_update
{
		...
		const std::vector<uint8_t> &pkgHdr, size_t offset)
	{
		...
		while (deviceIdRecCount-- && (pkgHdrRemainingSize > 0)) {
			...
			auto rc = decode_firmware_device_id_record(
				...
				componentBitmapBitLength, &deviceIdRecHeader,
				    ...
				    uintmax_t pkgSize)
	{
		...
		offset = parseFDIdentificationArea(deviceIdRecCount, pkgHdr,
						   offset);
