...
TEST(AttrTable, EnumEntryDecodeTest)
{
    ...
    rc =
        pldm_bios_table_attr_entry_enum_decode_vdn_num_check(nullptr, &vdnNumber);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);

    entry->attr_type = PLDM_BIOS_STRING;
    rc = pldm_bios_table_attr_entry_enum_decode_vdn_num_check(entry, &vdnNumber);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);

    rc =
        pldm_bios_table_attr_entry_enum_decode_vdn_hdls_check(entry, nullptr, 0);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
}
