...
def run(args):
    ...
    print(f"Running corpus", file=sys.stderr)
    for c in corpus.glob('*'):
        c = c.open('rb').read()
        subprocess.run([args.program], input=c)


    print(f"Running grcov", file=sys.stderr)
    outdir.mkdir(parents=True, exist_ok=True)
    coverage_paths = [args.builddir]
    lcov_file = outdir / 'lcov.info'

    # path_patch is needed because if the meson builddir is a subdirectory of the srcdir,
    # paths in the coverage .gcno are relative ../
    # It can be avoided by passing -fprofile-abs-path to gcc.
    #path_patch = [-p, '../']

    subprocess.run(['grcov',
        '-b', args.program,
        '-o', lcov_file,
        '-t', 'lcov',
        '-s', args.srcdir,
        # '-p', path_patch,
        ] + coverage_paths, check=True)

    print(f"Running genhtml", file=sys.stderr)
    subprocess.run(['genhtml',
        '-o', outdir,
        '--show-details',
        '--highlight',
