...
static int fuzz_decode_get_package_data_req(const struct pldm_msg* msg,
                                            size_t payload_length)
{
    struct pldm_get_package_data_req req;

    decode_get_package_data_req(msg, payload_length, &req);
...
static int fuzz_decode_pldm_rde_multipart_send_req(const struct pldm_msg* msg,
                                                   size_t payload_length)
{
    struct pldm_rde_multipart_send_req req;

    decode_pldm_rde_multipart_send_req(msg, payload_length, &req);

    return 0;
}

static int fuzz_decode_pldm_rde_multipart_send_resp(const struct pldm_msg* msg,
                                                    size_t payload_length)
{
    struct pldm_rde_multipart_send_resp resp;

    decode_pldm_rde_multipart_send_resp(msg, payload_length, &resp);
...
static int fuzz_encode_get_package_data_resp(struct pldm_msg* msg,
                                             size_t payload_length,
                                             const uint8_t* data, size_t size)
{
    struct pldm_get_package_data_resp resp_fixed;
    ...
    encode_get_package_data_resp(instance_id, &resp_fixed, &portion, msg,
                                 &payload_length);
                                                   ...
                                                   size_t size)
{
    ...
    if (tail_len)
    {
        locator_len = (uint8_t)(locator_len % (tail_len + 1));
    }
    else
    {
        locator_len = 0;
...
static int fuzz_encode_pldm_rde_operation_complete_req(struct pldm_msg* msg,
                                                       size_t payload_length,
                                                       const uint8_t* data,
                                                       size_t size)
{
    ...
    encode_pldm_rde_operation_complete_req(instance_id, &req, msg,
                                           &payload_length);

    return 0;
}

static int fuzz_encode_pldm_rde_operation_complete_resp(struct pldm_msg* msg,
                                                        size_t payload_length,
                                                        const uint8_t* data,
                                                        size_t size)
{
    ...
    encode_pldm_rde_operation_complete_resp(instance_id, &resp, msg,
                                            &payload_length);
...
static int fuzz_encode_pldm_rde_multipart_send_req(struct pldm_msg* msg,
                                                   size_t payload_length,
                                                   const uint8_t* data,
                                                   size_t size)
{
    struct pldm_rde_multipart_send_req req = {0};
    PLDM_MSGBUF_RO_DEFINE_P(buf);
    ...
    req.data.ptr = payload;
    req.data.length = payload_len;
    encode_pldm_rde_multipart_send_req(instance_id, &req, msg, &payload_length);

    return 0;
}

static int fuzz_encode_pldm_rde_multipart_send_resp(struct pldm_msg* msg,
                                                    size_t payload_length,
                                                    const uint8_t* data,
                                                    size_t size)
{
    ...
    encode_pldm_rde_multipart_send_resp(instance_id, &resp, msg,
                                        &payload_length);
...
static int (*const encode_pldm_msg_tests[])(struct pldm_msg*, size_t,
    ...
    fuzz_encode_pldm_rde_multipart_send_resp,
