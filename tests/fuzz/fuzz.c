...
static int
    fuzz_encode_pldm_platform_compact_numeric_sensor_pdr(const uint8_t* data,
                                                         size_t size)
{
    struct pldm_platform_compact_numeric_sensor_pdr pdr;
    uint8_t
        buf[PLDM_PLATFORM_COMPACT_NUMERIC_SENSOR_PDR_MIN_LENGTH + UINT8_MAX];
    size_t buf_len = sizeof(buf);
    ...
    memcpy(&pdr, data, sizeof(pdr));
    pdr.sensor_name.ptr = data + sizeof(pdr);
    pdr.sensor_name.length =
        size - sizeof(pdr) > UINT8_MAX ? UINT8_MAX : size - sizeof(pdr);
    encode_pldm_platform_compact_numeric_sensor_pdr(&pdr, buf, &buf_len);
...
static int
    fuzz_decode_pldm_platform_compact_numeric_sensor_pdr(const uint8_t* data,
                                                         size_t size)
{
    struct pldm_platform_compact_numeric_sensor_pdr pdr;

    decode_pldm_platform_compact_numeric_sensor_pdr(data, size, &pdr);
...
static uint8_t
    fuzz_pd_get_sensor_reading(void* ctx LIBPLDM_CC_UNUSED,
                               uint16_t sensor_id LIBPLDM_CC_UNUSED,
                               ...
                               struct pldm_platform_pd_sensor_state* state)
{
    ...
    state->sensor_data_size = PLDM_SENSOR_DATA_SIZE_UINT32;
...
static int (*const fuzz_tests[])(const uint8_t*, size_t) = {
    ...
    fuzz_encode_pldm_platform_compact_numeric_sensor_pdr,
    fuzz_decode_pldm_platform_compact_numeric_sensor_pdr,
};
