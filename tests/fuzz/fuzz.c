...
static int fuzz_decode_pldm_platform_set_state_sensor_enables_req(
    const struct pldm_msg* msg, size_t payload_length)
{
    struct pldm_platform_set_state_sensor_enables_req req;

    decode_pldm_platform_set_state_sensor_enables_req(msg, payload_length,
                                                      &req);
...
static int (*const decode_pldm_msg_tests[])(const struct pldm_msg*, size_t) = {
    ...
    fuzz_decode_pldm_platform_set_state_sensor_enables_req,
...
static int fuzz_encode_pldm_platform_set_state_sensor_enables_req(
    struct pldm_msg* msg, size_t payload_length, const uint8_t* data,
    size_t size)
{
    struct pldm_platform_set_state_sensor_enables_req req;
    ...
    pldm_msgbuf_extract(buf, instance_id);
    pldm_msgbuf_extract(buf, req.sensor_id);
    pldm_msgbuf_extract(buf, req.composite_sensor_count);
    for (size_t i = 0; i < ARRAY_SIZE(req.op_fields); i++)
    {
        pldm_msgbuf_extract(buf, req.op_fields[i].sensor_operational_state);
        pldm_msgbuf_extract(buf, req.op_fields[i].sensor_event_message_enable);
    }
    ...
    encode_pldm_platform_set_state_sensor_enables_req(instance_id, &req, msg,
                                                      &payload_length);
                                                      ...
                                                      size_t size)
{
    ...
    pdr.composite_sensor_count = data[2];
                                                           ...
                                                           size_t size)
{
    ...
    size_t avail = size - PLDM_PLATFORM_STATE_SENSOR_POSSIBLE_STATES_MIN_LENGTH;
    uint8_t cap = avail > UINT8_MAX ? UINT8_MAX : (uint8_t)avail;

    states.state_set_id = (uint16_t)(data[0] | (data[1] << 8));
    states.possible_states_size = cap ? (uint8_t)(data[2] % (cap + 1)) : 0;
    possible_states.ptr =
        data + PLDM_PLATFORM_STATE_SENSOR_POSSIBLE_STATES_MIN_LENGTH;
    possible_states.length = states.possible_states_size;
