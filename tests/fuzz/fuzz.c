...
static int
    fuzz_decode_pldm_rde_rde_operation_init_req(const struct pldm_msg* msg,
                                                size_t payload_length)
{
    struct pldm_rde_rde_operation_init_req req;

    decode_pldm_rde_rde_operation_init_req(msg, payload_length, &req);

    return 0;
}

static int
    fuzz_decode_pldm_rde_rde_operation_init_resp(const struct pldm_msg* msg,
                                                 size_t payload_length)
{
    struct pldm_rde_rde_operation_init_resp resp;

    decode_pldm_rde_rde_operation_init_resp(msg, payload_length, &resp);
...
static int
    fuzz_decode_pldm_rde_rde_operation_status_req(const struct pldm_msg* msg,
                                                  size_t payload_length)
{
    struct pldm_rde_rde_operation_status_req req;

    decode_pldm_rde_rde_operation_status_req(msg, payload_length, &req);

    return 0;
}

static int
    fuzz_decode_pldm_rde_rde_operation_status_resp(const struct pldm_msg* msg,
                                                   size_t payload_length)
{
    struct pldm_rde_rde_operation_status_resp resp;

    decode_pldm_rde_rde_operation_status_resp(msg, payload_length, &resp);
...
static int
    fuzz_decode_pldm_rde_rde_multipart_send_req(const struct pldm_msg* msg,
                                                size_t payload_length)
{
    struct pldm_rde_rde_multipart_send_req req;

    decode_pldm_rde_rde_multipart_send_req(msg, payload_length, &req);

    return 0;
}

static int
    fuzz_decode_pldm_rde_rde_multipart_send_resp(const struct pldm_msg* msg,
                                                 size_t payload_length)
{
    struct pldm_rde_rde_multipart_send_resp resp;

    decode_pldm_rde_rde_multipart_send_resp(msg, payload_length, &resp);

    return 0;
}

static int
    fuzz_decode_pldm_rde_rde_multipart_receive_req(const struct pldm_msg* msg,
                                                   size_t payload_length)
{
    struct pldm_rde_rde_multipart_receive_req req;

    decode_pldm_rde_rde_multipart_receive_req(msg, payload_length, &req);

    return 0;
}

static int
    fuzz_decode_pldm_rde_rde_multipart_receive_resp(const struct pldm_msg* msg,
                                                    size_t payload_length)
{
    struct pldm_rde_rde_multipart_receive_resp resp;

    decode_pldm_rde_rde_multipart_receive_resp(msg, payload_length, &resp);

    return 0;
}

static int (*const decode_pldm_msg_tests[])(const struct pldm_msg*, size_t) = {
    fuzz_decode_pldm_rde_rde_multipart_receive_req,
    fuzz_decode_pldm_rde_rde_multipart_receive_resp,
    fuzz_decode_pldm_rde_rde_multipart_send_req,
    fuzz_decode_pldm_rde_rde_multipart_send_resp,
    ...
    size_t size)
{
    ...
    resp.provider_name.length = name_len;
