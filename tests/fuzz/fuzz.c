...
static int fuzz_decode_pldm_platform_set_state_effecter_enables_req(
    const struct pldm_msg* msg, size_t payload_length)
{
    struct pldm_platform_set_state_effecter_enables_req req;

    decode_pldm_platform_set_state_effecter_enables_req(msg, payload_length,
                                                        &req);
    return 0;
}

static int fuzz_decode_pldm_platform_set_state_effecter_enables_resp(
    const struct pldm_msg* msg, size_t payload_length)
{
    struct pldm_platform_set_state_effecter_enables_resp resp;

    decode_pldm_platform_set_state_effecter_enables_resp(msg, payload_length,
                                                         &resp);
    return 0;
}
#endif
...
#if HAVE_LIBPLDM_API_TESTING
    fuzz_decode_pldm_platform_set_state_effecter_enables_req,
    fuzz_decode_pldm_platform_set_state_effecter_enables_resp,
#endif
};
...
#if HAVE_LIBPLDM_API_TESTING
static int fuzz_encode_pldm_platform_set_state_effecter_enables_req(
    struct pldm_msg* msg, size_t payload_length, const uint8_t* data,
    size_t size)
{
    struct pldm_platform_set_state_effecter_enables_req req;
    ...
    for (size_t index = 0; index < ARRAY_SIZE(req.op_fields); ++index)
    {
        pldm_msgbuf_extract(buf,
                            req.op_fields[index].effecter_operational_state);
        pldm_msgbuf_extract(buf, req.op_fields[index].event_msg_enable);
    }
    rc = pldm_msgbuf_complete(buf);
    if (rc)
    {
    ...
    encode_pldm_platform_set_state_effecter_enables_req(instance_id, &req, msg,
                                                        &payload_length);
    return 0;
}

static int fuzz_encode_pldm_platform_set_state_effecter_enables_resp(
    struct pldm_msg* msg, size_t payload_length, const uint8_t* data,
    size_t size)
{
    struct pldm_platform_set_state_effecter_enables_resp resp;
    ...
    encode_pldm_platform_set_state_effecter_enables_resp(instance_id, &resp,
                                                         msg, &payload_length);
    return 0;
}
#endif
...
#if HAVE_LIBPLDM_API_TESTING
    fuzz_encode_pldm_platform_set_state_effecter_enables_req,
    fuzz_encode_pldm_platform_set_state_effecter_enables_resp,
#endif
};
