...
TEST(GetNextCommandTIDTest, GetNextCommand)
{
    ...
    // The first byte of encoded PLDM message should be 10000001 -where 1 refers
    // to request on the most sig bit and the least sig bit refers to instance
    // id)
...
TEST(GetNextCommandPLDMTypesTest, GetNextCommand)
{
    ...
    // PLDM Header first byte - 100000001 (0x81 in hex)
...
TEST(PushResponseTypes, PushResponseInContext)
{
    ...
    uint8_t pldm_types[] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    auto rc =
        pldm_base_push_response(testContext, responsePtr, responseMsgSize);
    EXPECT_EQ(0, rc);
    for (int i = 0; i < 8; i++)
    {
        EXPECT_EQ(pldm_types[i], testContext->pldm_types[i]);
    }
}
