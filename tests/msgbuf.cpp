...
TEST(msgbuf, pldm_msgbuf_span_string_ascii_good)
{
    ...
    EXPECT_EQ(pldm_msgbuf_span_string_ascii(ctxExtract, (void**)&retBuff),
              PLDM_SUCCESS);
    EXPECT_EQ(pldm_msgbuf_extract_uint8(ctxExtract, &testVal1), PLDM_SUCCESS);
    EXPECT_EQ(0x77, testVal1);

    EXPECT_EQ(required, strlen(retBuff) + 1);
    EXPECT_EQ(memcmp(expectData, retBuff, strlen(retBuff) + 1), 0);
    EXPECT_EQ(pldm_msgbuf_destroy(ctxExtract), PLDM_SUCCESS);
}
...
TEST(msgbuf, pldm_msgbuf_span_string_utf16_mix)
{
    ...
    EXPECT_EQ(pldm_msgbuf_span_string_utf16(ctxExtract, (void**)&name1),
              PLDM_SUCCESS);
    EXPECT_EQ(memcmp(expectName1, name1, sizeof(expectName1)), 0);
