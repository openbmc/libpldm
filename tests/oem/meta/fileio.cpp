...
TEST(DecodeOemMetaFileIoReadReq, testGoodDecodeRequest)
{
    ...
    struct pldm_oem_meta_file_io_read_data_info data_info = {0};
    ...
    uint8_t buf[hdrSize + sizeof(uint8_t) + sizeof(uint8_t) + sizeof(struct pldm_oem_meta_file_io_read_data_info))] = {};
    ...
    pldm_msgbuf_insert_array_uint8(
        ctx, &data_info, sizeof(struct pldm_oem_meta_file_io_read_data_info));
