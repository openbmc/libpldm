...
TEST(PackageParser, ValidPkgSingleDescriptorSingleComponent)
{
	std::vector<uint8_t> fwPkgHdr{
		...
		0x08,
		0x00, // component bitmap bit length
		...
		0x02,
		0x00, // record 0: descriptor type: UUID

		0x10,
		0x00, // record 0: InitialDescriptorLength (16 bytes)
		...
		0x01,
		0x00, // component image count

		0x0A,
		0x00, // component classification
		0x64,
		0x00, // component identifier
		0xFF,
		0xFF,
		0xFF,
		0xFF, // component comparison stamp
		0x00,
		0x00, // component options

		0x00,
		0x00, // requested component activation method
		0x8B,
		0x00,
		0x00,
		0x00, // component location offset
		0x01,
		0x00,
		0x00,
		0x00, // component size
		...
		0x6E,
		0x67,
		0x33, // component version string

		0x54,
		0x9d,
		0x7d,
		0xe1, // package header checksum
	...
	std::vector<ComponentImageInfo> compImageInfos{
		{ 10, 100, 0xFFFFFFFF, 0, 0,
		  variable_field{ (uint8_t *)139, 1 }, "VersionString3" }
	};

	auto outfwDeviceIDRecords = res.value()->fwDeviceIDRecords;

	auto dd1 = DescriptorData{ std::vector<uint8_t>{
		0x16, 0x20, 0x23, 0xC9, 0x3E, 0xC5, 0x41, 0x15, 0x95, 0xF4,
		0x48, 0x70, 0x1D, 0x49, 0xD6, 0x75 } };
	...
	EXPECT_EQ(outCompImageInfos[0].compOptions,
		  compImageInfos[0].compOptions);
	...
	EXPECT_EQ(outCompImageInfos[0].compVersion,
		  compImageInfos[0].compVersion);
}
...
TEST(PackageParser, ValidPkgMultipleDescriptorsMultipleComponents)
{
	std::vector<uint8_t> fwPkgHdr{
		...
		0xA0, 0x2F, 0x05, 0x9A, 0xCA, 0x02, // UUID

		0x01,				    // header format revision
		0x46, 0x01,			    // pkg header size

		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x19, 0x0C,
		0xE5, 0x07,
		0x00,	    // pkg release date time, 13 bytes, timestamp104
		...
		0x69, 0x6E, 0x67, 0x31, // package version string
		...
		0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72,
		0x69, 0x6E, 0x67, 0x32, // component image set version string (14 bytes)

		0x02, 0x00, // record 0: record descriptor type: uuid
		,
		...
		0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72,
		0x69, 0x6E, 0x67, 0x35, // component version string (14 bytes)
		...
		0x0E,			// component version string length
		0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72,
		0x69, 0x6E, 0x67, 0x36, // component version string
		...
		0x0E,			// component version string length
		0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72,
		0x69, 0x6E, 0x67, 0x37, // component version string
	...
	std::vector<ComponentImageInfo> compImageInfos{
		{ 10, 100, 0xFFFFFFFF, 0, 0,
		  variable_field{ (uint8_t *)326, 1 }, "VersionString5" },
		{ 10, 200, 0xFFFFFFFF, 0, 1,
		  variable_field{ (uint8_t *)326, 1 }, "VersionString6" },
		{ 11, 300, 0xFFFFFFFF, 1, 12,
		  variable_field{ (uint8_t *)326, 1 }, "VersionString7" }
	};
	...
	auto dd1 = DescriptorData{ std::vector<uint8_t>{
		...
		0xFC, 0x8A, 0x56, 0x58, 0x7D, 0x5B } };
	auto dd2 = DescriptorData{ std::vector<uint8_t>{ 0x47, 0x16, 0x00,
							 0x00 } };
	...
	FirmwareDeviceIDRecord record1 = { std::bitset<32>(1),
					   { 0, 1 },
					   "VersionString2",
					   descriptorMap1,
					   {} };

	auto dd4 = DescriptorData{ std::vector<uint8_t>{
		0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18, 0xA0, 0x30,
		0xFC, 0x8A, 0x56, 0x58, 0x7D, 0x5C } };
	...
	auto dd5 = DescriptorData{ std::vector<uint8_t>{
		0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18, 0xA0, 0x30,
		0xFC, 0x8A, 0x56, 0x58, 0x7D, 0x5D } };
	...
	for (size_t i = 0; i < fwDeviceIDRecords.size(); i++) {
		...
		for (uint16_t type :
			...
			const auto d2 =
				fwDeviceIDRecords[i].getDescriptor(type);
...
TEST(PackageParser, InvalidPkgBadChecksum)
{
	std::vector<uint8_t> fwPkgHdr{
		0xF0, 0x18, 0x87, 0x8C, 0xCB, 0x7D, 0x49, 0x43, 0x98, 0x00,
		0xA0, 0x2F, 0x05, 0x9A, 0xCA, 0x02, 0x01, 0x8B, 0x00, 0x00,
		...
		0x01, 0x2E, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0E,
		0x00, 0x00, 0x01, 0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E,
		0x53, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x32, 0x02, 0x00, 0x10,
		0x00, 0x16, 0x20, 0x23, 0xC9, 0x3E, 0xC5, 0x41, 0x15, 0x95,
		0xF4, 0x48, 0x70, 0x1D, 0x49, 0xD6, 0x75, 0x01, 0x00, 0x0A,
		0x00, 0x64, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00,
		0x00, 0x8B, 0x00, 0x00, 0x00, 0x1B, 0x00, 0x00, 0x00, 0x01,
		0x0E, 0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74,
		0x72, 0x69, 0x6E, 0x67, 0x33, 0x4F, 0x96, 0xAE, 0x57
	};
