...
TEST(PackageParser, ValidPkgSingleDescriptorSingleComponent)
{
	std::vector<uint8_t> fwPkgHdr{
		...
		0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72,
		0x69, 0x6E, 0x67, 0x33, // component version string
	...
	auto res = PackageParser::parse(fwPkgHdr, pkgSize);
	...
	// we cannot compare the applicable components here since their memory address will be different
...
TEST(PackageParser, InvalidPkgBadChecksum)
{
	...
	std::expected<std::unique_ptr<Package>, PackageParserError> result =
		PackageParser::parse(fwPkgHdr, pkgSize);
