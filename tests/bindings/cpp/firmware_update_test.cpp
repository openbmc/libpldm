#include "libpldm/cpp/firmware_update.hpp"
#include "libpldm/cpp/types.hpp"
...
TEST(PackageParserTest, ValidPkgSingleDescriptorSingleComponent)
{
    ...
    EXPECT_EQ(outfwDeviceIDRecords[0].componentImageSetVersionString,
              fwDeviceIDRecords[0].componentImageSetVersionString);
    ...
    {
        const auto& d1 = outfwDeviceIDRecords[0].recordDescriptors.at(type);
        const auto& d2 = fwDeviceIDRecords[0].recordDescriptors.at(type);
        EXPECT_TRUE(*d1 == *d2);
    }
    ...
    EXPECT_EQ(outCompImageInfos[0].componentClassification,
              compImageInfos[0].componentClassification);
    EXPECT_EQ(outCompImageInfos[0].componentIdentifier,
              compImageInfos[0].componentIdentifier);
    EXPECT_EQ(outCompImageInfos[0].compComparisonStamp,
              compImageInfos[0].compComparisonStamp);
    EXPECT_EQ(outCompImageInfos[0].componentOptions,
              compImageInfos[0].componentOptions);
    EXPECT_EQ(outCompImageInfos[0].requestedComponentActivationMethod,
              compImageInfos[0].requestedComponentActivationMethod);
    EXPECT_EQ(outCompImageInfos[0].componentLocation.length,
              compImageInfos[0].componentLocation.length);
    EXPECT_EQ(outCompImageInfos[0].componentVersion,
              compImageInfos[0].componentVersion);
}
...
TEST(PackageParserTest, ValidPkgMultipleDescriptorsMultipleComponents)
{
    ...
    for (size_t i = 0; i < fwDeviceIDRecords.size(); i++)
    {
        ...
        EXPECT_EQ(outfwDeviceIDRecords[i].componentImageSetVersionString,
                  fwDeviceIDRecords[i].componentImageSetVersionString);
        ...
        {
            const auto& d1 = outfwDeviceIDRecords[i].recordDescriptors.at(type);
            const auto& d2 = fwDeviceIDRecords[i].recordDescriptors.at(type);
    ...
    {
        EXPECT_EQ(outCompImageInfos[i].componentClassification,
                  compImageInfos[i].componentClassification);
        EXPECT_EQ(outCompImageInfos[i].componentIdentifier,
                  compImageInfos[i].componentIdentifier);
        EXPECT_EQ(outCompImageInfos[i].compComparisonStamp,
                  compImageInfos[i].compComparisonStamp);
        EXPECT_EQ(outCompImageInfos[i].componentOptions,
                  compImageInfos[i].componentOptions);
        EXPECT_EQ(outCompImageInfos[i].requestedComponentActivationMethod,
                  compImageInfos[i].requestedComponentActivationMethod);
        EXPECT_EQ(outCompImageInfos[i].componentLocation.length,
                  compImageInfos[i].componentLocation.length);
        EXPECT_EQ(outCompImageInfos[i].componentVersion,
                  compImageInfos[i].componentVersion);
    }
}
