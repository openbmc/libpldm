...
TEST(PdSetup, NullArgs)
{
    ...
    EXPECT_EQ(
        ...
        pldm_platform_pd_setup(reinterpret_cast<pldm_platform_pd*>(buf),
...
TEST(PdSetSensorOps, NullOps)
{
    ...
    ASSERT_EQ(
        ...
        pldm_platform_pd_setup(reinterpret_cast<pldm_platform_pd*>(buf),
                               PLDM_SIZEOF_PLDM_PLATFORM_PD, pdr, nullptr),
        0);
    EXPECT_EQ(pldm_platform_pd_set_sensor_ops(
                  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
                  reinterpret_cast<pldm_platform_pd*>(buf), nullptr, nullptr),
              -EINVAL);
    pldm_pdr_destroy(pdr);
}
