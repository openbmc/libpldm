...
class PdTest : public ::testing::Test
{
  protected:
    ...
    uint32_t add_dummy_pdr(const std::vector<uint8_t>& data)
    {
        uint32_t handle = 0;
...
{
    std::vector<uint8_t> rec = {0xaa, 0xbb, 0xcc, 0xdd};
    add_dummy_pdr(rec);
    ...
    EXPECT_EQ(le32toh(r->largest_record_size), rec.size());
}

TEST_F(PdTest, GetPdrRepositoryInfoMultipleRecords)
{
    std::vector<uint8_t> short_rec = {0x01, 0x02};
    std::vector<uint8_t> long_rec = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
    add_dummy_pdr(short_rec);
    add_dummy_pdr(long_rec);
    add_dummy_pdr(short_rec);
    ...
    EXPECT_EQ(le32toh(r->largest_record_size), long_rec.size());
}
...
TEST_F(PdTest, GetPdrSingleRecordRoundtrip)
{
    ...
    uint32_t handle = add_dummy_pdr(payload);

    /* pldm_pdr_add overwrites payload[0..3] with the record handle (LE)
     * when the handle was auto-assigned. Retrieve the actual stored bytes. */
