...
TEST(PdSetup, NullArgs)
{
    ...
    /* NULL pdr */
    EXPECT_EQ(
        pldm_platform_pd_setup(std::start_lifetime_as<pldm_platform_pd>(buf),
...
TEST_F(PdTest, GetPdrRepositoryInfoEmptyRepo)
{
    ...
    auto* r = std::start_lifetime_as<const pldm_pdr_repository_info_resp>(
        resp->data() + hdrSize);
...
TEST_F(PdTest, GetPdrSingleRecordRoundtrip)
{
    ...
    auto* r =
        std::start_lifetime_as<const pldm_get_pdr_resp>(resp->data() + hdrSize);
...
TEST_F(PdTest, GetSensorReadingSuccess)
{
    ...
    auto* r = std::start_lifetime_as<const pldm_get_sensor_reading_resp>(
        resp->data() + hdrSize);
...
TEST(PdSetSensorOps, NullOps)
{
    ...
    EXPECT_EQ(
        pldm_platform_pd_set_sensor_ops(
            std::start_lifetime_as<pldm_platform_pd>(buf), nullptr, nullptr),
        -EINVAL);
    pldm_pdr_destroy(pdr);
}
...
TEST(PdControl, RegistersPlatformType)
{
    ...
    auto* control = std::start_lifetime_as<pldm_control>(ctrl_buf);
