...
struct SensorState
{
    ...
    uint8_t data_size = PLDM_SENSOR_DATA_SIZE_UINT8;
...
static uint8_t
    cb_get_sensor_reading(void* ctx, uint16_t sensor_id,
  ...
  protected:
    ...
    void SetUp() override
    {
        ...
        struct pldm_platform_pd_ops ops = {
            .get_sensor_reading = cb_get_sensor_reading, .ctx = &g_sensor};
        pd = pldm_platform_pd_new(pdr, nullptr, &ops, sizeof(ops));
...
TEST(PdSetup, NullArgs)
{
    ...
    /* NULL pd */
    EXPECT_EQ(pldm_platform_pd_setup(nullptr, PLDM_SIZEOF_PLDM_PLATFORM_PD, pdr,
                                     nullptr, &ops, sizeof(ops)),
              -EINVAL);

    /* NULL pdr */
    // NOLINTBEGIN(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(pldm_platform_pd_setup(reinterpret_cast<pldm_platform_pd*>(buf),
                                     PLDM_SIZEOF_PLDM_PLATFORM_PD, nullptr,
                                     nullptr, &ops, sizeof(ops)),
              -EINVAL);
...
TEST(PdSetup, SizeTooSmall)
{
    ...
    // NOLINTBEGIN(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(pldm_platform_pd_setup(reinterpret_cast<pldm_platform_pd*>(buf),
                                     PLDM_SIZEOF_PLDM_PLATFORM_PD - 1, pdr,
                                     nullptr, &ops, sizeof(ops)),
              -EINVAL);
    // NOLINTEND(cppcoreguidelines-pro-type-reinterpret-cast)
...
TEST(PdSetup, OptionalCallbacksMayBeNull)
{
    ...
    // NOLINTBEGIN(cppcoreguidelines-pro-type-reinterpret-cast)
    int rc = pldm_platform_pd_setup(reinterpret_cast<pldm_platform_pd*>(buf),
                                    PLDM_SIZEOF_PLDM_PLATFORM_PD, pdr, nullptr,
                                    &ops, sizeof(ops));
    // NOLINTEND(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(rc, 0);
...
TEST(PdSetup, OpsWithUnknownNonZeroFieldReturnsE2BIG)
{
    alignas(PLDM_ALIGNOF_PLDM_PLATFORM_PD)
        uint8_t buf[PLDM_SIZEOF_PLDM_PLATFORM_PD];
    pldm_pdr* pdr = pldm_pdr_init();
    ...
    /* Simulate a caller compiled against a future ops struct with an extra
     * non-zero field appended beyond what this library knows about. */
    struct
    {
        struct pldm_platform_pd_ops known;
        uint8_t future_field;
    } big_ops = {};
    big_ops.future_field = 0x01;

    // NOLINTBEGIN(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(pldm_platform_pd_setup(
                  reinterpret_cast<pldm_platform_pd*>(buf),
                  PLDM_SIZEOF_PLDM_PLATFORM_PD, pdr, nullptr,
                  reinterpret_cast<const pldm_platform_pd_ops*>(&big_ops),
                  sizeof(big_ops)),
              -E2BIG);
    // NOLINTEND(cppcoreguidelines-pro-type-reinterpret-cast)
...
TEST(PdSetup, OpsWithUnknownZeroFieldsSucceeds)
{
    alignas(PLDM_ALIGNOF_PLDM_PLATFORM_PD)
        uint8_t buf[PLDM_SIZEOF_PLDM_PLATFORM_PD];
    pldm_pdr* pdr = pldm_pdr_init();
    ...
    /* Extra zero tail is fine — caller disabled the unknown feature. */
    struct
    {
        struct pldm_platform_pd_ops known;
        uint8_t future_field;
...
TEST_F(PdTest, GetPdrInvalidHandle)
{
    ...
    auto req = make_request(PLDM_GET_PDR, &pdr_req, sizeof(pdr_req));
    auto resp = dispatch(pd, req);
...
{
    g_sensor.sensor_id = 1;
    g_sensor.data_size = PLDM_SENSOR_DATA_SIZE_UINT8;
    ...
    auto req = make_request(PLDM_GET_SENSOR_READING, &sreq, sizeof(sreq));
    auto resp = dispatch(pd, req);
...
{
    /* g_sensor.sensor_id defaults to 1 → mismatch with 999 makes the
     * callback return INVALID_SENSOR_ID */
    struct pldm_get_sensor_reading_req sreq{};
...
{
    g_sensor.sensor_id = 2;
    g_sensor.data_size = PLDM_SENSOR_DATA_SIZE_UINT32;
...
{
    g_sensor.sensor_id = 1;
    g_sensor.data_size = PLDM_SENSOR_DATA_SIZE_SINT8;
...
{
    g_sensor.sensor_id = 1;
    g_sensor.data_size = PLDM_SENSOR_DATA_SIZE_UINT16;
...
{
    g_sensor.sensor_id = 1;
    g_sensor.data_size = PLDM_SENSOR_DATA_SIZE_SINT32;
...
/* ===================================================================
 * 5. GET_SENSOR_READING with no callback
 * =================================================================== */
...
TEST(PdSensorOps, GetSensorReadingNoOpsReturnsUnsupportedCmd)
{
    ...
    struct pldm_platform_pd* pd =
        pldm_platform_pd_new(pdr, nullptr, &ops, sizeof(ops));
    ASSERT_NE(pd, nullptr);
    /* get_sensor_reading is NULL — unsupported */
...
TEST(PdControl, RegistersPlatformType)
{
    ...
    struct pldm_platform_pd_ops ops = {
        .get_sensor_reading = cb_get_sensor_reading, .ctx = &g_sensor};
    struct pldm_platform_pd* pd =
        pldm_platform_pd_new(pdr, control, &ops, sizeof(ops));
    ASSERT_NE(pd, nullptr);
