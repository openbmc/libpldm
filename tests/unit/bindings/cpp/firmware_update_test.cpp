...
TEST(PackageParserTest, ValidPkgSingleDescriptorWithDownstreamDevices)
{
    ...
    0x00, 0x00, 0x00, 0x00, // DownstreamDeviceUpdateOptionFlags
    ...
    0x56, 0x65, 0x72, 0x73, 0x69, 0x6F,
    0x6E, 0x53, 0x74, 0x72, 0x69, 0x6E,
    0x67, 0x31, // DownstreamDeviceSelfContainedActivationMinVersionStringo

    // DownstreamDeviceSelfContainedActivationMinVersionComparisonStamp
    // (does not exist in our case here)
    ...
    0x6E, 0x53, 0x74, 0x72, 0x69, 0x6E,
    0x67, 0x33,             // component version string
    0x00, 0x00, 0x00, 0x00 // ComponentOpaqueDataLength
        // clang-format on
    };
    ...
    auto res = PackageParser::parse(pkg, PackagePin::v2);
    ...
    EXPECT_EQ(dsDevice.downstreamDeviceUpdateOptionFlags, 0x00);

    EXPECT_EQ(dsDevice.downstreamDeviceSelfContainedActivationMinVersionString,
              "VersionString1");

    auto comparisonStamp =
        dsDevice
            .downstreamDeviceSelfContainedActivationMinVersionComparisonStamp;

    EXPECT_TRUE(comparisonStamp.has_value());

    if (comparisonStamp.has_value())
    {
        const auto& compStamp = comparisonStamp.value();

        EXPECT_TRUE(std::holds_alternative<uint32_t>(compStamp));

        if (std::holds_alternative<uint32_t>(compStamp))
        {
            EXPECT_EQ(std::get<uint32_t>(compStamp), 0x00);
        }
    }
    ...
    EXPECT_EQ(dsDevice.downstreamDeviceReferenceManifestData.size(), 8);

    const std::vector<uint8_t> expectedReferenceManifestData{
        0x03, 0x02, 0x1d, 0xa0, 0x31, 0x32, 0x33, 0x34};
    EXPECT_EQ(dsDevice.downstreamDeviceReferenceManifestData,
