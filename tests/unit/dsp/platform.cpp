...
TEST(SetNumericEffecterEnable, testGoodEncodeRequest)
{
    ...
    EXPECT_EQ(request->hdr.command, PLDM_SET_NUMERIC_EFFECTER_ENABLE);
...
TEST(EncodeSetNumericEffecterEnableResp, testGoodEncode)
{
    ...
    auto rc = encode_pldm_platform_set_numeric_effecter_enable_resp(
        instanceId, PLDM_SUCCESS, response, &payload_length);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(payload_length,
              PLDM_PLATFORM_SET_NUMERIC_EFFECTER_ENABLE_RESP_BYTES);

    EXPECT_EQ(response->hdr.command, PLDM_SET_NUMERIC_EFFECTER_ENABLE);
    EXPECT_EQ(response->hdr.type, PLDM_PLATFORM);
    EXPECT_EQ(response->hdr.request, 0);
    EXPECT_EQ(response->hdr.datagram, 0);
...
TEST(EffecterAuxiliaryNames, testDecodePdrGoodPath)
{
    ...
    foreach_pldm_platform_effecter_auxiliary_name(iter, effecter, rc)
    {
        ...
        foreach_pldm_platform_effecter_auxiliary_name_string(iter, effecter,
                                                             name, rc)
        {
            EXPECT_NE(name.tag.ptr, nullptr);
            EXPECT_GT(name.tag.length, 0u);
            EXPECT_NE(name.name.ptr, nullptr);
            EXPECT_GT(name.name.length, 0u);
            total_names++;
        }
        ASSERT_EQ(rc, 0);
...
TEST(EffecterAuxiliaryNames, testDecodePdrNameContents)
{
    ...
    foreach_pldm_platform_effecter_auxiliary_name(iter, effecter, rc)
    {
        ...
        foreach_pldm_platform_effecter_auxiliary_name_string(iter, effecter,
                                                             name, rc)
        {
            EXPECT_EQ(name.tag.length, 3u);
            EXPECT_EQ(memcmp(name.tag.ptr, "en", name.tag.length), 0);
            EXPECT_EQ(name.name.length, 6u);
        }
        ASSERT_EQ(rc, 0);
    }
    ASSERT_EQ(rc, 0);
...
TEST(EffecterAuxiliaryNames, testInnerIteratorNullParams)
{
    ...
    auto rc = decode_pldm_platform_effecter_auxiliary_name_string_from_iter(
        nullptr, &entry);
    EXPECT_EQ(rc, -EINVAL);
}
#endif
