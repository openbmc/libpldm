...
{
    int rc;
    struct pldm_platform_set_state_sensor_enables_req decoded;

    const std::array<uint8_t, hdrSize + 8> req
        // PLDM header
        {
            0x80, PLDM_PLATFORM, PLDM_SET_STATE_SENSOR_ENABLES,
            0x67, 0x45, // sensor ID 0x4567
            0x02,       // count
            0x01, 0x00, // opField 0
            0x02, 0x01, // opField 1
    ...
    // Good decode
    rc = decode_pldm_platform_set_state_sensor_enables_req(msg, 7, &decoded);
    EXPECT_EQ(rc, 0);
    EXPECT_EQ(decoded.sensor_id, 0x4567);
    EXPECT_EQ(decoded.composite_sensor_count, 2);
    EXPECT_EQ(decoded.op_fields[0].sensor_operational_state,
              PLDM_SET_SENSOR_DISABLED);
    EXPECT_EQ(decoded.op_fields[0].sensor_event_message_enable,
              PLDM_PLATFORM_SET_SENSOR_EVENT_MESSAGE_NO_CHANGE);
    EXPECT_EQ(decoded.op_fields[1].sensor_operational_state,
              PLDM_SET_SENSOR_UNAVAILABLE);
    EXPECT_EQ(decoded.op_fields[1].sensor_event_message_enable,
              PLDM_PLATFORM_SET_SENSOR_EVENT_MESSAGE_DISABLE_EVENTS);

    // Truncated part-way through an opField
    rc = decode_pldm_platform_set_state_sensor_enables_req(msg, 6, &decoded);
    EXPECT_EQ(rc, -EOVERFLOW);

    // Truncated on an opField boundary
    rc = decode_pldm_platform_set_state_sensor_enables_req(msg, 5, &decoded);
    EXPECT_EQ(rc, -EOVERFLOW);

    // Overlength message
    rc = decode_pldm_platform_set_state_sensor_enables_req(msg, 8, &decoded);
    EXPECT_EQ(rc, -EBADMSG);
}
...
#if HAVE_LIBPLDM_API_TESTING
TEST(SetStateSensorEnables, testDecodeBadRequest)
{
    int rc;
    struct pldm_platform_set_state_sensor_enables_req decoded;
    ...
    rc = decode_pldm_platform_set_state_sensor_enables_req(NULL, 5, &decoded);
    EXPECT_EQ(rc, -EINVAL);

    rc = decode_pldm_platform_set_state_sensor_enables_req(msg, 5, NULL);
    EXPECT_EQ(rc, -EINVAL);

    // Zero composite sensor count
    rc = decode_pldm_platform_set_state_sensor_enables_req(msg, 5, &decoded);
    EXPECT_EQ(rc, -EPROTO);

    // Excessive composite sensor count
    const std::array<uint8_t, hdrSize + 5> bigCountReq
        // PLDM header
        {
            0x80,
            PLDM_PLATFORM,
            ...
            0x67,
            0x45, // sensor ID 0x4567
            PLDM_PLATFORM_SET_STATE_SENSOR_ENABLES_MAX_COUNT + 1,
            0x01,
            0x00, // field 0
        };

    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    auto bigCountMsg = reinterpret_cast<const pldm_msg*>(bigCountReq.data());

    rc = decode_pldm_platform_set_state_sensor_enables_req(bigCountMsg, 5,
                                                           &decoded);
    EXPECT_EQ(rc, -EPROTO);
}
#endif // LIBPLDM_API_TESTING
...
TEST(SetStateSensorEnables, testEncodeRequestMaxCompositeSensorCount)
{
    ...
    for (auto& op_field : req.op_fields)
    {
        op_field.sensor_operational_state = PLDM_SET_SENSOR_ENABLED;
        op_field.sensor_event_message_enable = PLDM_STATE_EVENTS_ONLY_ENABLED;
    }
...
TEST(SetStateSensorEnables, testBadEncodeRequest)
{
    ...
    // Test zero composite sensor count
    struct pldm_platform_set_state_sensor_enables_req zeroCountReq = req;
    zeroCountReq.composite_sensor_count = 0;
    pl = maxLength;
    rc = encode_pldm_platform_set_state_sensor_enables_req(
        instanceID, &zeroCountReq, msg, &pl);
    EXPECT_EQ(rc, -EINVAL);

    // Test excessive composite sensor count
    struct pldm_platform_set_state_sensor_enables_req bigCountReq = req;
    bigCountReq.composite_sensor_count =
        PLDM_PLATFORM_SET_STATE_SENSOR_ENABLES_MAX_COUNT + 1;
    pl = maxLength;
    rc = encode_pldm_platform_set_state_sensor_enables_req(
    ...
    struct pldm_platform_set_state_sensor_enables_req invalidOpStateReq = req;
    invalidOpStateReq.op_fields[1].sensor_operational_state =
    ...
    struct pldm_platform_set_state_sensor_enables_req invalidEventReq = req;
    invalidEventReq.op_fields[1].sensor_event_message_enable =
...
TEST(SetStateSensorEnables, testGoodEncodeResponse)
{
    ...
    const std::array<uint8_t, PLDM_PLATFORM_SET_STATE_SENSOR_ENABLES_RESP_BYTES>
        expected{PLDM_ERROR_INVALID_DATA};
    EXPECT_EQ(memcmp(msg->payload, expected.data(), expected.size()), 0);
}
#endif // LIBPLDM_API_TESTING
...
TEST(SetStateSensorEnables, testBadEncodeResponse)
{
    ...
    rc =
        encode_pldm_platform_set_state_sensor_enables_resp(0, &resp, NULL, &pl);
    EXPECT_EQ(rc, -EINVAL);

    pl = PLDM_PLATFORM_SET_STATE_SENSOR_ENABLES_RESP_BYTES;
    rc = encode_pldm_platform_set_state_sensor_enables_resp(0, NULL, msg, &pl);
    EXPECT_EQ(rc, -EINVAL);

    rc =
        encode_pldm_platform_set_state_sensor_enables_resp(0, &resp, msg, NULL);
    EXPECT_EQ(rc, -EINVAL);

    pl = 0;
    rc = encode_pldm_platform_set_state_sensor_enables_resp(0, &resp, msg, &pl);
    EXPECT_EQ(rc, -EOVERFLOW);

    pl = PLDM_PLATFORM_SET_STATE_SENSOR_ENABLES_RESP_BYTES;
    rc = encode_pldm_platform_set_state_sensor_enables_resp(
        PLDM_INSTANCE_MAX + 1, &resp, msg, &pl);
    EXPECT_EQ(rc, -EINVAL);
}
#endif // LIBPLDM_API_TESTING
...
TEST(decodePldmPlatformStateSensorPdr, GoodTest)
{
    ...
    struct pldm_platform_state_sensor_pdr decodedPdr;
    auto rc = decode_pldm_platform_state_sensor_pdr(pdr.data(), pdr.size(),
                                                    &decodedPdr);
    EXPECT_EQ(0, rc);
    EXPECT_EQ(1u, decodedPdr.hdr.record_handle);
    EXPECT_EQ(1u, decodedPdr.hdr.version);
    ...
    EXPECT_EQ(PLDM_ENTITY_POWER_SUPPLY, decodedPdr.entity_type);
    EXPECT_EQ(1u, decodedPdr.entity_instance_number);
...
TEST(encodePldmPlatformStateSensorPossibleStates, SizeMismatchTest)
{
    ...
    // possibleStatesSize disagrees with the bitfield handed to the encoder
    states.state_set_id = 13;
    states.possible_states_size = 1;
    bits.ptr = bytes.data();
    bits.length = bytes.size();
...
TEST(StateSensorPDR, testMultipleEntries)
{
    ...
    pldm_platform_state_sensor_possible_states states{};
    int rc;
...
TEST(StateSensorPDR, testSingleEntry)
{
    ...
    {
        entryCount++;
        EXPECT_EQ(states.state_set_id, 42);
        EXPECT_EQ(states.possible_states_size, 3);
...
TEST(StateSensorPDR, testZeroPossibleStatesSize)
{
    ...
    {
        entryCount++;
        EXPECT_EQ(states.state_set_id, 100);
        EXPECT_EQ(states.possible_states_size, 0);
