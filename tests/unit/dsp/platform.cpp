...
TEST(SetNumericEffecterValue, testGoodEncodeRequestUint64)
{
    ...
    EXPECT_EQ(req->effecter_id, effecter_id);
...
TEST(GetNumericEffecterValue, testGoodEncodeDecodeRoundTripSint64)
{
    ...
    EXPECT_EQ(effecter_operState, reteffecter_operState);
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(pendingValue, *(reinterpret_cast<int64_t*>(retpendingValue)));
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(presentValue, *(reinterpret_cast<int64_t*>(retpresentValue)));
}
