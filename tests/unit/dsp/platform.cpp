...
TEST(encodeNumericSensorPdrData, RoundtripUint8)
{
    ...
    constexpr size_t pdr_len =
        PLDM_PDR_NUMERIC_SENSOR_PDR_FIXED_LENGTH + 3 * sensor_data_bytes +
        9 * range_field_bytes;
    ...
    EXPECT_EQ(decoded.sensor_auxiliary_names_pdr, pdr.sensor_auxiliary_names_pdr);
...
TEST(encodeNumericSensorPdrData, RoundtripSint32Real32)
{
    ...
    EXPECT_EQ(decoded.range_field_format, pdr.range_field_format);
    EXPECT_FLOAT_EQ(decoded.nominal_value.value_f32, pdr.nominal_value.value_f32);
    ...
    EXPECT_FLOAT_EQ(decoded.critical_high.value_f32, pdr.critical_high.value_f32);
...
TEST(encodeNumericSensorPdrData, NullArgs)
{
    ...
    EXPECT_EQ(encode_pldm_platform_numeric_sensor_value_pdr(nullptr, buf.data(),
                                                            &buf_len),
              -EINVAL);
    EXPECT_EQ(encode_pldm_platform_numeric_sensor_value_pdr(&pdr, nullptr,
                                                            &buf_len),
              -EINVAL);
