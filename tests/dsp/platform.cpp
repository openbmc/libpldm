...
TEST(GetEventReceiver, testGoodEncodeResponse)
{
    ...
    std::array<uint8_t,
               hdrSize + PLDM_GET_EVENT_RECEIVER_MIN_RESP_BYTES>
        responseMsg{};
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    auto response = reinterpret_cast<pldm_msg*>(responseMsg.data());
    auto rc = encode_get_event_receiver_resp(
        0, completionCode, transportProtocolType, eventReceiverAddressInfo,
        response);
    EXPECT_EQ(rc, PLDM_SUCCESS);
...
TEST(GetEventReceiver, testGoodDecodeResponse)
{
    ...
    auto rc = decode_get_event_receiver_resp(response, responseMsg.size(),
    &received_transport_protocol_type, &received_event_receiver_address_info,
    &received_completion_code);
    EXPECT_EQ(rc, PLDM_SUCCESS);
...
TEST(GetEventReceiver, testBadDecodeResponse)
{
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    reinterpret_cast<struct pldm_get_event_receiver_resp*>(response->payload);
    auto rc = decode_get_event_receiver_resp(nullptr, responseMsg.size()-sizeof(pldm_msg_hdr),
    &received_transport_protocol_type, &received_event_receiver_address_info,
    &received_completion_code);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
    rc = decode_get_event_receiver_resp(response, responseMsg.size()-sizeof(pldm_msg_hdr)-1,
    &received_transport_protocol_type, &received_event_receiver_address_info,
    &received_completion_code);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
    res->transport_protocol_type=1;
    rc = decode_get_event_receiver_resp(response, responseMsg.size()-sizeof(pldm_msg_hdr),
    &received_transport_protocol_type, &received_event_receiver_address_info,
    &received_completion_code);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
}
