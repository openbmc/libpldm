...
TEST(SetNumericEffecterEnable, testBadEncodeRequestInvalidOperationalState)
{
    struct pldm_platform_set_numeric_effecter_enable_req req = {
        .effecter_id = 0x1234,
        .effecter_operational_state =
            static_cast<enum pldm_effecter_oper_state>(
                EFFECTER_OPER_STATE_INTEST + 1)};
...
TEST(SetNumericEffecterEnable, testGoodEncodeRequestAllOperationalStates)
{
    ...
    std::array<enum pldm_effecter_oper_state, 4> valid_states = {
    ...
    for (auto state : valid_states)
    {
        ...
        std::array<uint8_t, PLDM_PLATFORM_SET_NUMERIC_EFFECTER_ENABLE_REQ_BYTES>
            expectedPayload = {0xCD, 0xAB, static_cast<uint8_t>(state)};
        EXPECT_EQ(0,
                  memcmp(request->payload, expectedPayload.data(),
                         PLDM_PLATFORM_SET_NUMERIC_EFFECTER_ENABLE_REQ_BYTES));
    }
}
#endif
...
TEST(EncodeSetNumericEffecterEnableResp, testGoodEncodeAllCompletionCodes)
{
    ...
    for (auto test_cc : test_codes)
    {
        size_t payload_length =
            PLDM_PLATFORM_SET_NUMERIC_EFFECTER_ENABLE_RESP_BYTES;
        auto rc = encode_pldm_platform_set_numeric_effecter_enable_resp(
            0, test_cc, response, &payload_length);

        ASSERT_EQ(rc, 0);
        ASSERT_EQ(payload_length,
                  PLDM_PLATFORM_SET_NUMERIC_EFFECTER_ENABLE_RESP_BYTES);
        EXPECT_EQ(response->payload[0], test_cc);
    }
}
#endif
...
TEST(EffecterAuxiliaryNames, testDecodePdrGoodPath)
{
    ...
    auto rc = decode_pldm_effecter_auxiliary_names_pdr(
...
TEST(EffecterAuxiliaryNames, testDecodePdrNullParams)
{
    ...
    auto rc = decode_pldm_effecter_auxiliary_names_pdr(nullptr, 1, &pdr, &iter);
    EXPECT_EQ(rc, -EINVAL);

    rc = decode_pldm_effecter_auxiliary_names_pdr(&data, 1, nullptr, &iter);
    EXPECT_EQ(rc, -EINVAL);

    rc = decode_pldm_effecter_auxiliary_names_pdr(&data, 1, &pdr, nullptr);
    EXPECT_EQ(rc, -EINVAL);
}
#endif
...
TEST(EffecterAuxiliaryNames, testDecodePdrNoEffecters)
{
    std::vector<uint8_t> pdr_data{
        // Common PDR Header
        0x01, 0x00, 0x00, 0x00, 0x01, PLDM_EFFECTER_AUXILIARY_NAMES_PDR, 0x01,
        0x00, 0x00, 0x00, // dataLength (placeholder)
        // Fixed fields
        0x01, 0x00, // terminusHandle
        0x02, 0x00, // effecterID
...
TEST(EffecterAuxiliaryNames, testIteratorInsufficientData)
{
    std::vector<uint8_t> pdr_data{
        ...
        0x02, 0x00, // effecterID
        0x01,       // effecterCount = 1
        // Effecter claiming 2 names but no data follows
...
#ifdef LIBPLDM_API_TESTING
TEST(EffecterAuxiliaryNames, testIteratorNullDataPtr)
{
    struct pldm_effecter_auxiliary_names_iter iter = {};
    iter.count = 1;
    ...
    auto rc = decode_pldm_effecter_auxiliary_name_from_iter(&iter, &name);
    EXPECT_EQ(rc, -EINVAL);
}
#endif
