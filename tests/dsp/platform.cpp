...
TEST(decodeEntityAuxNamePdrData, GoodTest)
{
    ...
    const char16_t expectName0[4] = {0x0053, 0x0030, 0x0053, 0x0000};
    const char16_t expectName1[3] = {0x0053, 0x0031, 0x0000};
    const char16_t expectName2[4] = {0x0052, 0x0052, 0x0033, 0x0000};
    ...
    EXPECT_EQ(3, strlen(decodedPdr->names[0].name_language_tag) + 1);
    EXPECT_EQ(0, memcmp(decodedPdr->names[0].name_language_tag, expectTag0,
                        strlen(decodedPdr->names[0].name_language_tag)));
    EXPECT_EQ(4, char16len(decodedPdr->names[0].entity_aux_name) + 1);
    EXPECT_EQ(
        0, memcmp(decodedPdr->names[0].entity_aux_name, expectName0,
                  sizeof(char16_t) *
                      (char16len(decodedPdr->names[0].entity_aux_name) + 1)));

    EXPECT_EQ(3, strlen(decodedPdr->names[1].name_language_tag) + 1);
    EXPECT_EQ(0, memcmp(decodedPdr->names[1].name_language_tag, expectTag1,
                        strlen(decodedPdr->names[1].name_language_tag)));
    EXPECT_EQ(3, char16len(decodedPdr->names[1].entity_aux_name) + 1);
    EXPECT_EQ(
        0, memcmp(decodedPdr->names[1].entity_aux_name, expectName1,
                  sizeof(char16_t) *
                      (char16len(decodedPdr->names[1].entity_aux_name) + 1)));

    EXPECT_EQ(3, strlen(decodedPdr->names[2].name_language_tag) + 1);
    EXPECT_EQ(0, memcmp(decodedPdr->names[2].name_language_tag, expectTag2,
                        strlen(decodedPdr->names[2].name_language_tag)));
    EXPECT_EQ(4, char16len(decodedPdr->names[2].entity_aux_name) + 1);
    EXPECT_EQ(
        0, memcmp(decodedPdr->names[2].entity_aux_name, expectName2,
                  sizeof(char16_t) *
                      (char16len(decodedPdr->names[2].entity_aux_name) + 1)));
    free(decodedPdr);
}
#endif
