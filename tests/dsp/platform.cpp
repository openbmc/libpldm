...
{
    // Allocate PLDM message with payload size = 3 bytes
    ...
    auto rc = decode_get_state_sensor_readings_resp(
        &response.hdr, sizeof(response), &completionCode, &count, nullptr);
...
{
    // Payload size = 1 (too short)
    PLDM_MSG_DEFINE_P(response, 1);
    ...
    auto rc = decode_get_state_sensor_readings_resp(
        &response.hdr, sizeof(response), &cc, &count, nullptr);
