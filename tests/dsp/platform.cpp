...
TEST(PlatformEventMessage, testGoodCperEventDataDecodeRequest)
{
     ...
     * event_data_length
     */
    size_t cper_data_length = eventData.size() - PLDM_CPER_EVENT_MIN_LENGTH;

    auto cper_event = reinterpret_cast<struct pldm_platform_cper_event*>(
        malloc(sizeof(struct pldm_platform_cper_event) + cper_data_length));
...
TEST(PlatformEventMessage, testBadCperEventDataDecodeRequest)
{
    ...
    auto rc = decode_pldm_cper_event_data(NULL, eventData.size(), cper_event);
    EXPECT_EQ(rc, -EINVAL);
