...
TEST(EntityAssociationPDR, testFindParentEntityPositiveAndNegative)
{
    ...
    auto secondChild = pldm_entity_association_tree_add(
        tree, &entities[2], 0xffff, child, PLDM_ENTITY_ASSOCIAION_PHYSICAL);
    ...
    EXPECT_EQ(pldm_entity_association_pdr_add_from_node_with_record_handle(
                  child, repo, &entities, 3, false, 1, 20),
              0);
    ...
    EXPECT_EQ(pldm_entity_association_find_parent_entity(
                  nullptr, nullptr, false, &record_handle, &found),
              -EINVAL);
