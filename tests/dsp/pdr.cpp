...
TEST(StateEffecterPDR, testExtractPossibleStates)
{
    ...
    foreach_pldm_pdr_state_effecter_possible_states(pdr, pdrSize, states, rc)
    {
...
TEST(StateEffecterPDR, testInvalidBufferTooSmall)
{
    ...
    foreach_pldm_pdr_state_effecter_possible_states(pdr, 10, states, rc)
    {
...
TEST(StateEffecterPDR, testZeroPossibleStatesSize)
{
    ...
    pldm_msgbuf_insert_uint16(buf, 100);
    pldm_msgbuf_insert_uint8(buf, 0); // Invalid: must be >= 1
