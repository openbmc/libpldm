...
TEST(EntityAssociationPDR, testPDRWithRecordHandle)
{
    ...
    uint32_t record_handle_from_pdr = 0;
    uint8_t hdr_type;
    auto rc =
    ...
    pldm_msgbuf_extract_uint8(buf, &hdr_type);
    EXPECT_EQ(hdr_type, PLDM_PDR_ENTITY_ASSOCIATION);

    pldm_msgbuf_span_required(buf, sizeof(uint16_t), (void**)&skip_data);

    uint16_t hdr_length;
    pldm_msgbuf_extract_uint16(buf, &hdr_length);
    EXPECT_EQ(hdr_length, size - sizeof(struct pldm_pdr_hdr));

    uint16_t hdr_container_id;
    pldm_msgbuf_extract_uint16(buf, &hdr_container_id);
    EXPECT_EQ(hdr_container_id, 1u);

    uint8_t hdr_association_type;
    pldm_msgbuf_extract_uint8(buf, &hdr_association_type);
    EXPECT_EQ(hdr_association_type, PLDM_ENTITY_ASSOCIAION_LOGICAL);

    uint16_t container_entity_type;
    uint16_t container_entity_instance_num;
    uint16_t container_entity_container_id;

    pldm_msgbuf_extract_uint16(buf, &container_entity_type);
    pldm_msgbuf_extract_uint16(buf, &container_entity_instance_num);
    pldm_msgbuf_extract_uint16(buf, &container_entity_container_id);
    EXPECT_EQ(container_entity_type, 1u);
    EXPECT_EQ(container_entity_instance_num, 1u);
    EXPECT_EQ(container_entity_container_id, 0u);

    uint8_t num_children;
    pldm_msgbuf_extract_uint8(buf, &num_children);
    EXPECT_EQ(num_children, 1u);

    uint16_t child_entity_type;
    uint16_t child_entity_instance_num;
    uint16_t child_entity_container_id;
    for (int i = 0; i < num_children; ++i)
    {
        pldm_msgbuf_extract_uint16(buf, &child_entity_type);
        pldm_msgbuf_extract_uint16(buf, &child_entity_instance_num);
        pldm_msgbuf_extract_uint16(buf, &child_entity_container_id);
        EXPECT_EQ(child_entity_type, 3u);
        EXPECT_EQ(child_entity_instance_num, 1u);
        EXPECT_EQ(child_entity_container_id, 1u);
    }
    ...
    pldm_msgbuf_extract_uint32(buf, &record_handle_from_pdr);
    EXPECT_EQ(record_handle_from_pdr, 11u);
    uint8_t version = 0;
    pldm_msgbuf_extract_uint8(buf, &version);
    ...
    pldm_msgbuf_extract_uint16(buf, &record_change_num);
    ...
    pldm_msgbuf_extract_uint8(buf, &hdr_association_type);
    EXPECT_EQ(hdr_association_type, PLDM_ENTITY_ASSOCIAION_PHYSICAL);
    ...
    for (int i = 0; i < num_children; ++i)
    {
        ...
        pldm_msgbuf_extract_uint16(buf, &child_entity_instance_num);
        pldm_msgbuf_extract_uint16(buf, &child_entity_container_id);
        EXPECT_EQ(child_entity_type, 2u);
        EXPECT_EQ(child_entity_instance_num, 1u);
        EXPECT_EQ(child_entity_container_id, 1u);
    }
