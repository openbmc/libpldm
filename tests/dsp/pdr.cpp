...
TEST(EntityAssociationPDR, testPDRWithRecordHandle)
{
    ...
    pldm__msgbuf_extract_uint8(buf, &num_children);
    EXPECT_EQ(num_children, 1u);

    for (int i = 0; i < num_children; ++i)
    {
        struct pldm_entity e;
        pldm__msgbuf_extract_uint16(buf, &e.entity_type);
        pldm__msgbuf_extract_uint16(buf, &e.entity_instance_num);
        pldm__msgbuf_extract_uint16(buf, &e.entity_container_id);
        EXPECT_EQ(e.entity_type, 3u);
        EXPECT_EQ(e.entity_instance_num, 1u);
        EXPECT_EQ(e.entity_container_id, 1u);
    }
    ...
    uint8_t version = 0;
    pldm__msgbuf_extract_uint8(buf, &version);
    ...
    pldm__msgbuf_extract_uint16(buf, &record_change_num);
    ...
    for (int i = 0; i < num_children; ++i)
    {
        ...
        pldm__msgbuf_extract_uint16(buf, &e.entity_instance_num);
        pldm__msgbuf_extract_uint16(buf, &e.entity_container_id);
        EXPECT_EQ(e.entity_type, 2u);
        EXPECT_EQ(e.entity_instance_num, 1u);
        EXPECT_EQ(e.entity_container_id, 1u);
    }

    pldm_pdr_destroy(repo);
    pldm_entity_association_tree_destroy_check(tree);
}
