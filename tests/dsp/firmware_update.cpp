...
TEST(UpdateSecurityRevision, goodPathEncodeRequest)
{
    ...
    std::array<uint8_t, hdrSize + PLDM_UPDATE_SECURITY_REVISION_REQ_BYTES>
        outRequest{0x83, 0x05, 0x16, 0x05, 0x00, 0x02, 0x01, 0x42};
    EXPECT_EQ(request, outRequest);

#ifdef LIBPLDM_API_TESTING
    /* Check the roundtrip */
    struct pldm_update_security_revision_req req;
    PLDM_MSG_DEFINE_P(dec, outRequest.size());
    std::copy(outRequest.begin(), outRequest.end(), dec_buf);
    rc = decode_update_security_revision_req(dec, outRequest.size() - hdrSize,
                                             &req);
    ASSERT_EQ(rc, 0);

    EXPECT_EQ(req.comp_classification, compClassification);
    EXPECT_EQ(req.comp_identifier, compIdentifier);
    EXPECT_EQ(req.comp_classification_index, compClassificationIndex);
#endif
}
