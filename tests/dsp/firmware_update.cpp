...
TEST(QueryDownstreamIdentifiers, goodPathDecodeResponse)
{
    ...
    std::array<struct pldm_downstream_device, number_of_downstream_devices_resp>
        devices{};
    std::vector<struct pldm_fw_descriptors> descriptors(total_descriptors);
    ...
    rc = decode_query_downstream_identifiers_index(
        ...
        descriptors.data(), total_descriptors * sizeof(pldm_fw_descriptors));
    for (const auto& device : devices)
    {
        EXPACT_EQ(device.downstream_device_index, downstream_device_index);
        EXPACT_EQ(device.downstream_descriptor_count,
                  downstream_descriptor_count);
        for (size_t i = 0; i < device.downstream_descriptor_count; i++)
        {
            EXPACT_EQ(device.downstream_descriptors[i].descriptor_type,
                      descriptor_type);
            EXPACT_EQ(device.downstream_descriptors[i].descriptor_length,
                      descriptor_length);
            EXPACT_EQ(device.downstream_descriptors[i].descriptor_data,
                      descriptors.data());
        }
    }
}
