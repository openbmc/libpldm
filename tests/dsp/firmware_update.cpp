...
TEST(DecodeFirmwareDeviceIdRecordV130, goodPath)
{
                                                ...
                                                deviceIdRec, descriptors, rc)
    {
        ...
        EXPECT_EQ(true, std::equal(deviceIdRec.applicable_components.ptr,
                                   ...
                                   applicableComponentsBitfield.begin(),
                                   applicableComponentsBitfield.end()));
        const auto compImageSetVersionStr = new (const_cast<void*>(
            deviceIdRec.component_image_set_version_string)) char
        ...
        foreach_pldm_descriptor(descriptors, descriptor, rc)
        {
            ...
            const auto descriptorData =
                new (const_cast<void*>(descriptor.descriptor_data))
                    uint8_t[descriptor.descriptor_length];
            EXPECT_EQ(true,
        ...
        EXPECT_EQ(true, std::equal(
                            ...
                            fwDevicePkgData.begin(), fwDevicePkgData.end()));
        const auto outRefManifestData =
            new (const_cast<void*>(deviceIdRec.reference_manifest_data))
                uint8_t[deviceIdRec.reference_manifest_length];
        EXPECT_EQ(true,
...
TEST(DecodeDownstreamDeviceIdRecordV130, goodPath)
{
        ...
        pkgHeaderInfo, dsDeviceIdRecs, dsDeviceIdRec, dsDescriptors, rc)
    {
        ...
        const auto outDsDevSelfContainedActMinVerStr = new (const_cast<void*>(
            dsDeviceIdRec
                .downstream_device_self_contained_activation_min_version_string)) char
            [dsDeviceIdRec
                 .downstream_device_self_contained_activation_min_version_string_length];
        EXPECT_EQ(
            true,
        ...
        EXPECT_EQ(rc, 0);
        const auto outDsDevicePkgData = new (
            const_cast<void*>(dsDeviceIdRec.downstream_device_package_data))
            uint8_t[dsDeviceIdRec.downstream_device_package_data_length];
        EXPECT_EQ(
            true,
        ...
        const auto outDsRefManifestData = new (const_cast<void*>(
            dsDeviceIdRec.downstream_device_reference_manifest_data))
            uint8_t[dsDeviceIdRec.downstream_device_reference_manifest_length];
        EXPECT_EQ(
            true,
            std::equal(
