...
TEST(RequestDownstreamDeviceUpdate, goodPathEncodeRequest)
{
    ...
    std::array<uint8_t, hdrSize + PLDM_DOWNSTREAM_DEVICE_UPDATE_REQUEST_BYTES> request{};
    ...
    std::array<uint8_t, hdrSize + PLDM_DOWNSTREAM_DEVICE_UPDATE_REQUEST_BYTES> outRequest{
...
TEST(RequestDownstreamDeviceUpdate, errorPathEncodeRequest)
{
    ...
    auto rc = encode_request_downstream_device_update_req(instanceId, nullptr, requestMsg,
                                                  &enc_payload_len);
    EXPECT_EQ(rc, -EINVAL);
    ...
    EXPECT_EQ(rc, PLDM_SUCCESS);
		...
		enc_payload_len = static_cast<size_t>(PLDM_DOWNSTREAM_DEVICE_UPDATE_REQUEST_BYTES) - 1;
    rc = encode_request_downstream_device_update_req(instanceId, &req_data,
                                                  requestMsg, &enc_payload_len);
    EXPECT_EQ(rc, -EINVAL);
		enc_payload_len = static_cast<size_t>(PLDM_DOWNSTREAM_DEVICE_UPDATE_REQUEST_BYTES);
    rc = encode_request_downstream_device_update_req(instanceId, &req_data,
                                                  requestMsg, &enc_payload_len);
    EXPECT_EQ(rc, PLDM_SUCCESS);
    ...
    rc = encode_request_downstream_device_update_req(instanceId, &req_data,
                                                  requestMsg, &enc_payload_len);
    EXPECT_EQ(rc, -EINVAL);
    req_data.maximum_downstream_device_transfer_size = PLDM_FWUP_BASELINE_TRANSFER_SIZE;
...
TEST(RequestDownstreamDeviceUpdate, goodPathDecodeResponse)
{
    ...
    auto rc = decode_request_downstream_device_update_resp(
        responseMsg1, PLDM_DOWNSTREAM_DEVICE_UPDATE_RESPONSE_BYTES, &resp_data1);
    EXPECT_EQ(rc, PLDM_SUCCESS);
    EXPECT_EQ(resp_data1.completion_code, PLDM_SUCCESS);
    EXPECT_EQ(resp_data1.downstream_device_meta_data_length, ddMetaDataLen);
    EXPECT_EQ(resp_data1.downstream_device_will_send_get_package_data, ddWillSendPkgData);
    ...
    rc = decode_request_downstream_device_update_resp(
        responseMsg2, PLDM_DOWNSTREAM_DEVICE_UPDATE_RESPONSE_BYTES, &resp_data2);
    EXPECT_EQ(rc, -EAGAIN);
}
...
TEST(RequestDownstreamDeviceUpdate, errorPathDecodeResponse)
{
    ...
    rc = decode_request_downstream_device_update_resp(responseMsg, 0, &resp_data);
    EXPECT_EQ(rc, -EINVAL);
}
