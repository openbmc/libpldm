...
static constexpr std::array<uint8_t, PLDM_FWUP_UUID_LENGTH>
    PLDM_FWUP_PACKAGE_HEADER_IDENTIFIER_V3_0{0x7b, 0x29, 0x1c, 0x99, 0x6d, 0xb6, 
...
TEST(DecodePldmFirmwareUpdatePackage, v1h1fd1fdd1cii)
{
    ...
    static const pldm_package_component_image_information expected_info{
        0x000a, 0x0000, 0xffffffff, {0}, {1}, {nullptr, 1}, 0x01, {nullptr, 0}, {nullptr, 0}};
...
#ifdef LIBPLDM_API_TESTING
TEST(DecodePldmFirmwareUpdatePackage, v3h1fd1fdd1cii)
{
    const std::array<uint8_t, 170> package{
        0x7B, 0x29, 0x1C, 0x99, 0x6D, 0xB6, 0x42, 0x08, 0x80, 0x1B,
        0x02, 0x02, 0x6E, 0x46, 0x3C, 0x78, 0x04, 0xA8, 0x00, 0x00,
        0xe9, 0x07, 0x03, 0x0b, 0x16, 0x03, 0x00, 0x00, 0x00, 0x00,
        0x76, 0x02, 0x08, 0x00, 0x01, 0x04, 't',  'e',  's',  't',

        0x01, 0x1C, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x04,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 'v',  '0',  '.', 
        '1',  0x01, 0x00, 0x04, 0x00, 0x9c, 0x01, 0x00, 0x00,

        0x01, 0x1C, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x04,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 'v',  '1',  '.', 
        '0',  0x01, 0x00, 0x04, 0x00, 0x9c, 0x01, 0x00, 0x00,

        0x02, 0x00,

        0x0a, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00,
        0x01, 0x00, 0xA8, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x01, 0x04, 'v',  '0',  '.',  '2',  0x00, 0x00, 0x00, 0x00,

        0x0a, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00,
        0x01, 0x00, 0xA9, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x01, 0x04, 'v',  '2',  '.',  '0',  0x00, 0x00, 0x00, 0x00,
    ...
    EXPECT_EQ(memcmp(PLDM_FWUP_PACKAGE_HEADER_IDENTIFIER_V3_0.data(),
                     hdr.package_header_identifier,
                     PLDM_FWUP_PACKAGE_HEADER_IDENTIFIER_V3_0.size()),
              0);
    ...
    foreach_pldm_package_firmware_device_id_record(iter, fdrec, rc)
    {
        ...
        EXPECT_NE(fdrec.record_descriptors.ptr, nullptr);
        ASSERT_EQ(fdrec.firmware_device_package_data.length, 0);
        ASSERT_EQ(fdrec.reference_manifest_data.length, 0);
    ...
    foreach_pldm_package_downstream_device_id_record(iter, ddrec, rc)
    {
        ...
        EXPECT_NE(ddrec.record_descriptors.ptr, nullptr);
        EXPECT_EQ(ddrec.package_data.length, 0);
        EXPECT_EQ(ddrec.reference_manifest_data.length, 0);
    ...
    foreach_pldm_package_component_image_information(iter, info, rc)
    {
        ...
        EXPECT_EQ(memcmp(version, info.component_version_string.ptr,
                         info.component_version_string.length),
                  0);
        EXPECT_EQ(info.component_opaque_data.length, expected->component_opaque_data.length);
        EXPECT_EQ(info.component_opaque_data.ptr, expected->component_opaque_data.ptr);
