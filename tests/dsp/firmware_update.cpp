...
TEST(PackageParser, ValidPkgSingleDescriptorSingleComponent)
{
    ...
    int rc =
        pldm_fwup_parse_package(fwPkgHdr.data(), fwPkgHdr.size(), &pkgData);
    EXPECT_EQ(rc, 0);
    ASSERT_NE(pkgData, nullptr);
    EXPECT_EQ(pkgData->pkg_header.package_header_size, fwPkgHdr.size());
    ASSERT_NE(pkgData->package_version_str, nullptr);
    EXPECT_EQ(std::string_view(pkgData->package_version_str), pkgVersion);
    EXPECT_EQ(pkgData->device_id_records_count, 1u);

    auto& firstRecord = pkgData->device_id_records[0];
    ASSERT_NE(firstRecord.comp_image_set_version, nullptr);
    EXPECT_STREQ(firstRecord.comp_image_set_version, "VersionString2");
    EXPECT_EQ(firstRecord.descriptors.desc_count, 1u);
    EXPECT_EQ(pkgData->comp_image_infos_count, 1u);

    auto& firstComp = pkgData->comp_image_infos[0];
    ASSERT_NE(firstComp.comp_version, nullptr);
    EXPECT_STREQ(firstComp.comp_version, "VersionString3");

    pldm_fwup_free_package_data(pkgData);
}
...
TEST(PackageParser, ValidPkgMultipleDescriptorsMultipleComponents)
{
    ...
    ASSERT_NE(pkgData->package_version_str, nullptr);
    EXPECT_EQ(std::string_view(pkgData->package_version_str), pkgVersion);
    ASSERT_EQ(pkgData->device_id_records_count, 3u);
    {
        const auto& rec = pkgData->device_id_records[0];
        EXPECT_EQ(rec.device_update_option_flags, 1u);
        ASSERT_EQ(rec.num_applicable_components, 2u);
        EXPECT_EQ(rec.applicable_components[0], 0u);
        EXPECT_EQ(rec.applicable_components[1], 1u);
        ASSERT_NE(rec.comp_image_set_version, nullptr);
        EXPECT_STREQ(rec.comp_image_set_version, "VersionString2");
        EXPECT_EQ(rec.descriptors.desc_count, 3u);
        {
            const auto& d = rec.descriptors.desc_array[0];
            EXPECT_EQ(d.descriptor_type, PLDM_FWUP_UUID);
            std::vector<uint8_t> expected_uuid{
                0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18,
                0xA0, 0x30, 0xFC, 0x8A, 0x56, 0x58, 0x7D, 0x5B};
            compareByteVector(d.descriptor_data, d.descriptor_length,
                              expected_uuid);
        }
        {
            const auto& d = rec.descriptors.desc_array[1];
            EXPECT_EQ(d.descriptor_type, PLDM_FWUP_IANA_ENTERPRISE_ID);
            std::vector<uint8_t> expected_iana{0x47, 0x16, 0x00, 0x00};
            compareByteVector(d.descriptor_data, d.descriptor_length,
                              expected_iana);
        }
        {
            const auto& d = rec.descriptors.desc_array[2];
            EXPECT_EQ(d.descriptor_type, PLDM_FWUP_VENDOR_DEFINED);
            EXPECT_EQ(d.is_vendor_defined, true);
            std::cout << d.vendor_title;
            EXPECT_EQ(std::string(d.vendor_title), "OpenBMC");
            std::vector<uint8_t> expected_iana{0x12, 0x34};
            compareByteVector(d.descriptor_data, d.descriptor_length,
                              expected_iana);
        }
    }
    {
        const auto& rec = pkgData->device_id_records[1];
        EXPECT_EQ(rec.device_update_option_flags, 0u);
        ASSERT_EQ(rec.num_applicable_components, 3u);
        EXPECT_EQ(rec.applicable_components[0], 0u);
        EXPECT_EQ(rec.applicable_components[1], 1u);
        EXPECT_EQ(rec.applicable_components[2], 2u);
        ASSERT_NE(rec.comp_image_set_version, nullptr);
        EXPECT_STREQ(rec.comp_image_set_version, "VersionString3");
        EXPECT_EQ(rec.descriptors.desc_count, 1u);
        {
            const auto& d = rec.descriptors.desc_array[0];
            EXPECT_EQ(d.descriptor_type, PLDM_FWUP_UUID);
            std::vector<uint8_t> expected_uuid{
    ...
    {
        const auto& rec = pkgData->device_id_records[2];
        EXPECT_EQ(rec.device_update_option_flags, 0u);
        ASSERT_EQ(rec.num_applicable_components, 1u);
        EXPECT_EQ(rec.applicable_components[0], 0u);
        ASSERT_NE(rec.comp_image_set_version, nullptr);
        EXPECT_STREQ(rec.comp_image_set_version, "VersionString4");
        EXPECT_EQ(rec.descriptors.desc_count, 1u);
        {
            const auto& d = rec.descriptors.desc_array[0];
            EXPECT_EQ(d.descriptor_type, PLDM_FWUP_UUID);
    ...
    auto& comp_infos = pkgData->comp_image_infos;
    auto comp_count = pkgData->comp_image_infos_count;
    ASSERT_EQ(comp_count, 3u);
    {
        const auto& cinfo = comp_infos[0];
        EXPECT_EQ(cinfo.comp_classification, 10u);
        EXPECT_EQ(cinfo.comp_identifier, 100u);
        EXPECT_EQ(cinfo.comp_comparison_stamp, 0xFFFFFFFFu);
        EXPECT_EQ(cinfo.comp_options, 0u);
        EXPECT_EQ(cinfo.requested_comp_activation_method, 0u);
        EXPECT_EQ(cinfo.comp_location_offset, 326u);
        EXPECT_EQ(cinfo.comp_size, 27u);
        ASSERT_NE(cinfo.comp_version, nullptr);
        EXPECT_STREQ(cinfo.comp_version, "VersionString5");
    }
    {
        const auto& cinfo = comp_infos[1];
        EXPECT_EQ(cinfo.comp_classification, 10u);
        EXPECT_EQ(cinfo.comp_identifier, 200u);
        EXPECT_EQ(cinfo.comp_comparison_stamp, 0xFFFFFFFFu);
        EXPECT_EQ(cinfo.comp_options, 0u);
        EXPECT_EQ(cinfo.requested_comp_activation_method, 1u);
        EXPECT_EQ(cinfo.comp_location_offset, 353u);
        EXPECT_EQ(cinfo.comp_size, 27u);
        ASSERT_NE(cinfo.comp_version, nullptr);
        EXPECT_STREQ(cinfo.comp_version, "VersionString6");
    }
    {
        const auto& cinfo = comp_infos[2];
        EXPECT_EQ(cinfo.comp_classification, 16u);
        EXPECT_EQ(cinfo.comp_identifier, 300u);
        EXPECT_EQ(cinfo.comp_comparison_stamp, 0xFFFFFFFFu);
        EXPECT_EQ(cinfo.comp_options, 1u);
        EXPECT_EQ(cinfo.requested_comp_activation_method, 12u);
        EXPECT_EQ(cinfo.comp_location_offset, 380u);
        EXPECT_EQ(cinfo.comp_size, 27u);
        ASSERT_NE(cinfo.comp_version, nullptr);
        EXPECT_STREQ(cinfo.comp_version, "VersionString7");
    }
    pldm_fwup_free_package_data(pkgData);
}
...
TEST_F(PackageParserMultipleDescSameType, DescriptorsMatch)
{
    ...
    ASSERT_NE(pkgData->package_version_str, nullptr);
    EXPECT_EQ(std::string_view(pkgData->package_version_str), pkgVersion);
    ASSERT_EQ(pkgData->device_id_records_count, 1u);
    const auto& rec = pkgData->device_id_records[0];
    EXPECT_EQ(rec.device_update_option_flags, 1u);
    ASSERT_NE(rec.comp_image_set_version, nullptr);
    EXPECT_STREQ(rec.comp_image_set_version, "VersionString2");
    ASSERT_EQ(rec.descriptors.desc_count, 5u);
    {
        const auto& d = rec.descriptors.desc_array[0];
        EXPECT_EQ(d.descriptor_type, PLDM_FWUP_UUID);
        std::vector<uint8_t> expectedUuid{0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D,
                                          0x47, 0x18, 0xA0, 0x30, 0xFC, 0x8A,
                                          0x56, 0x58, 0x7D, 0x5B};
        compareByteVector(d.descriptor_data, d.descriptor_length, expectedUuid);
    }
    {
        const auto& d = rec.descriptors.desc_array[1];
        EXPECT_EQ(d.descriptor_type, PLDM_FWUP_IANA_ENTERPRISE_ID);
        std::vector<uint8_t> expectedIana{0x47, 0x16, 0x00, 0x00};
        compareByteVector(d.descriptor_data, d.descriptor_length, expectedIana);
    }
    {
        const auto& d = rec.descriptors.desc_array[2];
        EXPECT_EQ(d.descriptor_type, PLDM_FWUP_IANA_ENTERPRISE_ID);
        std::vector<uint8_t> expectedIana2{0x47, 0x16, 0x10, 0x00};
        compareByteVector(d.descriptor_data, d.descriptor_length,
                          expectedIana2);
    }
    {
        const auto& d = rec.descriptors.desc_array[3];
        EXPECT_EQ(d.descriptor_type, PLDM_FWUP_VENDOR_DEFINED);

        ASSERT_NE(d.vendor_title, nullptr);
        EXPECT_STREQ(d.vendor_title, "OpenBMC");

        std::vector<uint8_t> expectedVendor{0x12, 0x34};
        compareByteVector(d.descriptor_data, d.descriptor_length,
                          expectedVendor);
    }
    {
        const auto& d = rec.descriptors.desc_array[4];
        EXPECT_EQ(d.descriptor_type, PLDM_FWUP_VENDOR_DEFINED);

        ASSERT_NE(d.vendor_title, nullptr);
        EXPECT_STREQ(d.vendor_title, "SKU");

        std::vector<uint8_t> expectedVendor{0x12, 0x34, 0x56};
        compareByteVector(d.descriptor_data, d.descriptor_length,
                          expectedVendor);
    }
}
