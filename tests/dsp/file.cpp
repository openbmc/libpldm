...
TEST(EncodeDfOpenReq, GoodTest)
{
    ...
    auto rc = encode_df_open_req(instance_id, file_identifier, file_attribute,
                                 requestPtr, PLDM_DF_OPEN_REQ_BYTES);
...
TEST(EncodeDfOpenReq, BadTest)
{
    ...
    PLDM_MSG_DEFINE_P(requestPtr, PLDM_DF_OPEN_REQ_BYTES);
    auto rc = encode_df_open_req(instance_id, file_identifier, file_attribute,
                                 nullptr, PLDM_DF_OPEN_REQ_BYTES);
    EXPECT_EQ(rc, -EINVAL);
    rc = encode_df_open_req(instance_id, file_identifier, file_attribute,
                            requestPtr, 1);
