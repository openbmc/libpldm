...
TEST(MultipartReceive, testGoodEncodeRequest)
{
    ...
    auto rc = encode_multipart_receive_req(instance_id, pldm_type,
                  transfer_opflag, transfer_ctx, transfer_handle,
...
TEST(MultipartReceive, testBadEncodeRequest)
{
    ...
    auto rc = encode_multipart_receive_req(instance_id, pldm_type,
                  ...
                  PLDM_MULTIPART_RECEIVE_REQ_BYTES);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
    rc = encode_multipart_receive_req(instance_id, pldm_type,
                  transfer_opflag, transfer_ctx, transfer_handle,
                  section_offset , section_length, requestPtr,
                  1);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
...
TEST(MultipartReceive, testGoodDecodeResponse)
{
    ...
    uint8_t transferFlag = PLDM_XFER_START_AND_END;
    uint32_t nextDataTransferHandle = 0x11;
    uint32_t dataSize = 9;
    ...
    std::vector<uint8_t> responseMsg(sizeof(pldm_msg_hdr) +
                                     ...
                                     dataSize + sizeof(dataIntegrityChecksum));

    auto responsePtr = reinterpret_cast<pldm_msg*>(responseMsg.data());
    size_t payload_length = responseMsg.size() - sizeof(pldm_msg_hdr);
    auto response =
	reinterpret_cast<pldm_multipart_receive_resp*>(responsePtr->payload);

    response->completion_code = completionCode;
    response->transfer_opflag = transferFlag;
    response->next_transfer_handle = htole32(nextDataTransferHandle);
    response->data_length = htole32(dataSize);
    response->data_integrity_checksum = htole32(dataIntegrityChecksum);
    memcpy(response->data, data.data(), dataSize);
    ...
    auto rc = decode_multipart_receive_resp(
                 responsePtr, payload_length, &retCompletionCode,
                 &retTransferFlag,  &retNextDataTransferHandle, &retDataSize,
                 &retData, &retDataIntegrityChecksum);
...
TEST(MultipartReceive, testBadDecodeResponse)
{
    ...
    auto rc = decode_multipart_receive_resp(
                 nullptr, payload_length, &retCompletionCode,
                 &retTransferFlag,  &retNextDataTransferHandle, &retDataSize,
                 &retData, &retDataIntegrityChecksum);
    ...
    rc = decode_multipart_receive_resp(
                 responsePtr, 0, &retCompletionCode,
                 &retTransferFlag,  &retNextDataTransferHandle, &retDataSize,
                 &retData, &retDataIntegrityChecksum);
