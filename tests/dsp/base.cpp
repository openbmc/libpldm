...
TEST(MultipartReceive, testGoodEncodeRequest)
{
    ...
    const struct pldm_multipart_receive_req req_data = {
        pldm_type,       transfer_opflag, transfer_ctx,
        transfer_handle, section_offset,  section_length};
...
TEST(MultipartReceive, testGoodDecodeResponse)
{
    ...
    size_t respDataSize =
        sizeof(struct pldm_multipart_receive_resp) + dataLength;
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    auto resp_data = reinterpret_cast<struct pldm_multipart_receive_resp*>(
        malloc(respDataSize));
    ...
    EXPECT_EQ(resp_data->data_length, dataLength);
    EXPECT_EQ(respDataIntegrityChecksum, dataIntegrityChecksum);

    auto respData = pldm_multipart_receive_resp_data(resp_data);
    EXPECT_NE(respData, nullptr);
    if (respData)
    {
        EXPECT_EQ(0, memcmp(data.data(), respData, dataLength));
    }
