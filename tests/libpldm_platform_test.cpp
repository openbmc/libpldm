...
/** @struct pldm_poll_for_platform_event_message_req_pack
 *
...
TEST(PollForPlatformEventMessage, testGoodEncodeRequest)
{
    ...
    struct pldm_poll_for_platform_event_message_req_pack* req =
        reinterpret_cast<struct pldm_poll_for_platform_event_message_req_pack*>(
            request->payload);
...
TEST(PollForPlatformEventMessage, testGoodDecodeRespond)
{
    ...
    [[maybe_unused]] uint8_t eventData[5] = {0x11, 0x22, 0x33, 0x44, 0x55};
    ...
    struct pldm_poll_for_platform_event_message_resp decodeResponseData = {};
    auto rc = decode_poll_for_platform_event_message_resp(
        response, responseMsg.size() - hdrSize, &decodeResponseData);

    EXPECT_EQ(rc, PLDM_SUCCESS);
    EXPECT_EQ(completionCode, decodeResponseData.completion_code);
    EXPECT_EQ(tId, decodeResponseData.tid);
    EXPECT_EQ(eventId, decodeResponseData.event_id);
    EXPECT_EQ(nextDataTransferHandle,
              decodeResponseData.next_data_transfer_handle);
    EXPECT_EQ(transferFlag, decodeResponseData.transfer_flag);
    EXPECT_EQ(eventClass, decodeResponseData.event_class);
    EXPECT_EQ(eventDataSize, decodeResponseData.event_data_size);
    // EXPECT_EQ(0,
    //           memcmp(eventData, decodeResponseData.event_data,
    //           eventDataSize));
    EXPECT_EQ(eventDataIntegrityChecksum,
              decodeResponseData.event_data_integrity_checksum);
}
...
TEST(PollForPlatformEventMessage, testBadDecodeRespond)
{
    ...
    struct pldm_poll_for_platform_event_message_resp decodeResponseData = {};
    auto rc = decode_poll_for_platform_event_message_resp(nullptr, 0,
                                                          &decodeResponseData);
