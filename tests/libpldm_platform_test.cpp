...
TEST(decodeNumericSensorPdrData, Uint8Test)
{
    ...
    EXPECT_FLOAT_EQ(1.0, decodedPdr.resolution);
    EXPECT_FLOAT_EQ(0, decodedPdr.offset);
    ...
    EXPECT_FLOAT_EQ(1.0, decodedPdr.state_transition_interval);
    EXPECT_FLOAT_EQ(1.0, decodedPdr.update_interval);
...
TEST(decodeNumericSensorPdrData, InvalidSizeTest)
{
    ...
    auto rc =
        decode_numeric_sensor_pdr_data(pdr1.data(), pdr1.size(), &decodedPdr);
    EXPECT_EQ(PLDM_ERROR_INVALID_LENGTH, rc);
}
