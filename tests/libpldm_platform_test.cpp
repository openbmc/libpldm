...
TEST(GetStateEffecterStates, testGoodEncodeRequest)
{
    ...
    auto rc = encode_get_state_effecter_states_req(0, effecter_id, request, PLDM_GET_STATE_EFFECTER_STATES_REQ_BYTES);
...
TEST(GetStateEffecterStates, testBadEncodeRequest)
{
    ...
    auto rc = encode_get_state_effecter_states_req(0, 0, nullptr, PLDM_GET_STATE_EFFECTER_STATES_REQ_BYTES);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
}
