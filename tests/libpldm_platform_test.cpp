...
TEST(decodeNumericEffecterPdrData, Real32TestOnlyNominalValue)
{
    std::vector<uint8_t> pdr1{
        0x1,
        0x0,
    ...
    EXPECT_EQ(-100000, decodedPdr.min_settable.value_s32);
    EXPECT_EQ(PLDM_RANGE_FIELD_FORMAT_REAL32, decodedPdr.range_field_format);
    EXPECT_EQ(0x01, decodedPdr.range_field_support.byte);
    EXPECT_FLOAT_EQ(0, decodedPdr.nominal_value.value_f32);
}

TEST(decodeNumericEffecterPdrData, Real32TestOnlyNormalMax)
{
    std::vector<uint8_t> pdr1{
        0x1,
        0x0,
    ...
    EXPECT_EQ(-100000, decodedPdr.min_settable.value_s32);
    EXPECT_EQ(PLDM_RANGE_FIELD_FORMAT_REAL32, decodedPdr.range_field_format);
    EXPECT_EQ(0x02, decodedPdr.range_field_support.byte);
    EXPECT_FLOAT_EQ(50.05f, decodedPdr.normal_max.value_f32);
}

TEST(decodeNumericEffecterPdrDataDeathTest, InvalidSizeTest)
{
    // A corrupted PDR. The data after plusTolerance missed.
    std::vector<uint8_t> pdr1{
        0x1,
