...
TEST(GetStateEffecterStates, testEncodeAndDecodeResponse)
{
    ...
    std::array<uint8_t,
        ...
        expectedResponseMsg{{0x81, PLDM_PLATFORM,
                             PLDM_GET_STATE_EFFECTER_STATES, completionCode,
                             comp_effecterCnt,
    ...
    auto rc = encode_get_state_effecter_states_resp(
        0, PLDM_SUCCESS, comp_effecterCnt, stateField.data(), response,
        responseMsg.size() - hdrSize);
    ...
    rc = decode_get_state_effecter_states_resp(
        response,
        responseMsgBad.size() - hdrSize - PLDM_GET_EFFECTER_STATE_FIELD_SIZE,
