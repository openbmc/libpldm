...
- platform: Added 64-bit sensor support per PLDM Type 2 v1.3.0 (DSP0248)
  ...
  - Added 64-bit sensor reading encode/decode support with proper endianness
    conversion
  - Added `decode_numeric_sensor_data_ex()` function for decoding numeric sensor
    event data with full 64-bit support without truncation. This replaces the
    legacy `decode_numeric_sensor_data()` which truncates 64-bit values to 32 bits
...
- platform: Added 64-bit effecter support per PLDM Type 2 v1.3.0 (DSP0248)
  ...
  - Note: `decode_set_numeric_effecter_value_req()` and
    `decode_get_numeric_effecter_value_resp()` remain limited to 32-bit due to
    fixed-size buffer parameters in their API
