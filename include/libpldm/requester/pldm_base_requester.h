...
/**
 * @brief Gets the next PLDM command in sequence to be executed for PLDM Discovery
 *
 * @param[in] ctx - pointer to a context which is to be initialized
...
/**
 * @brief Gets current status of the command that is to be processed
 *
 * @param[in] ctx - pointer to context which holds the command type and status
 * @param[out] pldm_type - Stores the `next_command` attribute value from the
 * context
 * @param[out] status - Stores the status of the `next_command` attribute value
 * from the context
 *
 * @return pldm_requester_rc_t (errno may be set)
 */
...
pldm_base_get_current_status(struct requester_base_context *ctx,
			     uint8_t *pldm_type, status_t *status);

/**
 * @brief Pushes the response values to the context attributes based on the command type
 * that was executed and updates the command status. It also sets the
