...
/**
 * @brief Unregisters all transports associated with the pldm instance
 *
 * @param[in] ctx - pldm requester instance
 *
 * @return pldm_requester_rc_t (errno may be set)
 ...
 *
 * @pre The pldm requester instance must be initialised and have a registered
 * 	transport; otherwise, PLDM_REQUESTER_INVALID_SETUP is returned. This
 * 	should be called after pldm_send_msg has been called.
 *
 * @return pldm_requester_rc_t (errno may be set)
 */
pldm_requester_rc_t pldm_requester_poll(struct pldm_requester *ctx,
					int timeout);
 ...
 * @param[out] instance_id - caller owned pointer to PLDM instance ID. Return
 * 	       PLDM_REQUESTER_INVALID_SETUP if this is NULL. On success, this
 * 	       value of the instance ID the message was sent with.
 ...
 *
 *
 * @param[in] ctx - pldm requester instance with a registered transport
 * @param[in] tid - destination PLDM TID
 * @param[in] pldm_req_msg - caller owned pointer to PLDM request msg or async
 * 	      notification. Behaviour undefined if this is not a valid PLDM msg.
 * @param[in] req_msg_len - size of PLDM request msg. If this is less than the
