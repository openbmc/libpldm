...
struct pldm_transport {
	const char *name;
	uint8_t version;
	int socket; // is this specific to mctp transports? or will all transports have this?
	pldm_requester_rc_t (*open)(struct pldm_transport *b);
	pldm_requester_rc_t (*recv)(struct pldm_transport *b, mctp_eid_t eid, uint8_t **pldm_resp_msg, size_t *resp_msg_len);
	pldm_requester_rc_t (*send)(struct pldm_transport *b, mctp_eid_t eid, const uint8_t *pldm_req_msg, size_t req_msg_len);
};
...
/**
 * @brief Registers transport with pldm core
 *
...
int pldm_unregister_transport(struct pldm *pldm, struct pldm_transport *transport);
 ...
 * @return pldm_requester_rc_t (errno may be set)
 */
pldm_requester_rc_t pldm_send_recv_msg(struct pldm_transport *transport, mctp_eid_t eid,
				       const uint8_t *pldm_req_msg,
				       size_t req_msg_len, uint8_t **pldm_resp_msg,
				       size_t *resp_msg_len);
 ...
 * @return pldm_requester_rc_t (errno may be set)
 */
pldm_requester_rc_t pldm_send_msg(struct pldm_transport *backed, mctp_eid_t eid,
				  const uint8_t *pldm_req_msg, size_t req_msg_len);
 ...
 *         when data was read, but didn't match eid or instance_id.
 */
pldm_requester_rc_t pldm_recv_msg(struct pldm_transport *transport, mctp_eid_t eid, uint8_t instance_id,
				  uint8_t **pldm_resp_msg, size_t *resp_msg_len);
 ...
 *         when data was read, but wasn't a PLDM response message
 */
pldm_requester_rc_t pldm_recv_msg_any_inst(struct pldm_transport *transport, mctp_eid_t eid,
				  	   uint8_t **pldm_resp_msg,
				  	   size_t *resp_msg_len);
