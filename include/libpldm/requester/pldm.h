 ...
 *
 * @pre The pldm requester core must be initialised and have a registered
 * 	transport. Otherwise PLDM_REQUESTER_SEND_FAIL is returned. If the
 ...
 * @param[in] pldm_req_msg - caller owned pointer to PLDM request or async msg.
 * 	      Behaviour undefined if this is not a valid PLDM msg.
 * @param[in] req_msg_len - size of PLDM request msg. Behaviour undefined if
 * 	      this is not the correct length of PLDM msg.
 ...
 * 	      minimum size of a PLDM msg an error is returned. Otherwise, if
 * 	      this is not the correct length of the PLDM msg, behaviour is
 * 	      undefined.
 *
 * @return pldm_requester_rc_t (errno may be set)
 */
...
/**
 * @brief Asynchronously get a PLDM response message matching the supplied tid
 * 	  and instance ID. Control is immediately returned to the caller.
 *
 * @pre The pldm requester core must be initialised and have a registered
 * 	transport. Otherwise PLDM_REQUESTER_RECV_FAIL is returned. If the
 * 	transport requires a TID to transport specific identifier mapping, this
 * 	must already be set up.
 *
 * @param[in] pldm - pldm requester core
 * @param[in] tid - destination PLDM TID
 * @param[in] instance_id - PLDM instance id of previously sent PLDM request or
 * 	      async msg that we are expecting a response to. If there is no
 * 	      message available with this instance ID an error is returned.
 * @param[out] pldm_resp_msg - *pldm_resp_msg will point to PLDM response msg if
 * 	       return code is PLDM_REQUESTER_SUCCESS, otherwise NULL. On success
 * 	       this function allocates memory, caller to free(*pldm_resp_msg).
 * @param[out] resp_msg_len - caller owned pointer that will be made point to
 *             the size of the PLDM response msg.
 *
 * @return pldm_requester_rc_t (errno may be set). Return success only if the
 * 	   message available is a PLDM response message that matches the tid and
