...
enum pldm_base_multipart_receive_transfer_flag {
	PLDM_BASE_MULTIPART_RECEIVE_XFER_FLAG_START = 0x01,
	PLDM_BASE_MULTIPART_RECEIVE_XFER_FLAG_MIDDLE = 0x02,
	PLDM_BASE_MULTIPART_RECEIVE_XFER_FLAG_END = 0x04,
	PLDM_BASE_MULTIPART_RECEIVE_XFER_FLAG_START_AND_END = 0x05,
	PLDM_BASE_MULTIPART_RECEIVE_XFER_FLAG_ACK_COMPLETION = 0x08,
};
...
/** @struct pldm_base_multipart_receive_min_resp
 *
 *  Structure representing PLDM multipart receive minimum response.
 */
struct pldm_base_multipart_receive_min_resp {
 ...
 *  @param[in] data_transfer_handle - The handle of data
 *  @param[in] req_section_offset - The start of the requested section
 *  @param[in] req_section_length - The length of the requested section
 *  @param[in,out] msg - Message will be written to this
 *  @param[in] payload_length - length of request message payload
 *  @return pldm_completion_codes
 */
int encode_base_multipart_receive_req(
	uint8_t instance_id, uint8_t pldm_type, uint8_t transfer_opflag,
	uint32_t transfer_ctx, uint32_t data_transfer_handle,
	uint32_t req_section_offset, uint32_t req_section_length,
 ...
 *  @param[out] transfer_opflag - Transfer Flag
 *  @param[out] transfer_handle - The handle of data
 *  @param[out] data_length - The length of response data
 *  @param[out] data - The pointer points into
 *      msg.payload and therefore has the same lifetime as msg.payload.
 *  @param[out] data_integrity_checksum - The checksum of response data
 *  @return pldm_completion_codes
 *  @note  Caller is responsible for memory alloc and dealloc of param
 *     'msg.payload'
 */
