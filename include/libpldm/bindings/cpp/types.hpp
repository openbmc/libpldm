...
namespace fw_update
{
	// Definitions of type aliases used for pldm packages.
	// To represent the packge structure, we can use a class hierarchy or
	// a nesting of STL containers. Using STL containers relieves us from needing
	// to define constructors but prevents proper naming, so we define some type
	// aliases here.
	...
	using ComponentImageInfo =
		std::tuple<CompClassification, CompIdentifier,
			   CompComparisonStamp, CompOptions,
