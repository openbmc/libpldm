 ...
 *          are not allocated, -EOVERFLOW if the provided output message
 *          buffer is too short to be parsed into the output request struct.
 ...
 *  @return 0 on success, -EINVAL if the input parameters' memory
 *          are not allocated, -EOVERFLOW if the provided input
 *          message buffer is too short for the output response struct.
 */
