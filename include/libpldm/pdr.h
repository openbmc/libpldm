 ...
 *  @param[in] is_remote - if true, then the PDR is not from this terminus
 *  @param[out] record_handle - if non-NULL, then record handle of the sensor PDR deleted
 *
 *  @return 0 on success, with the record handle of the deleted sensor PDR stored in 
 *  record_handle if record_handle is non-NULL.
 *  else returns -EINVAL when repo is NULL
 *  or -ENOENT if the  sensor id is not found in the repo.
 */
