...
struct pldm_platform_pd_sensor_state {
	...
	uint8_t event_state;
	/** PLDM_SENSOR_DATA_SIZE_* value describing which member of
	 *  current_reading was populated. */
...
struct pldm_platform_pd_ops {
	 ...
	 *
	 *  The callback is responsible for validating sensor_id, returning
	 *  PLDM_PLATFORM_INVALID_SENSOR_ID if it does not recognise it, and
	 *  for setting state->sensor_data_size on success. */
	uint8_t (*get_sensor_reading)(
		void *ctx, uint16_t sensor_id, bool8_t rearm_event_state,
 ...
 *  @param[in] pd_size      - Applications should pass PLDM_SIZEOF_PLDM_PLATFORM_PD, to
