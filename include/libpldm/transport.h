 ...
 *
 * pldm_transport_send_recv() will discard messages received on the socket that are not a response
 * that matches the request. Do not use this function if you're attempting to use the socket
 * asynchronously, as this discard behaviour will affect other responses that you may care about.
 *
 * @pre The pldm transport instance must be initialised; otherwise,
 * 	PLDM_REQUESTER_INVALID_SETUP is returned. If the transport requires a
 * 	TID to transport specific identifier mapping, this must already be set
