...
struct pldm_downstream_device_parameter_entry_versions {
	...
	/* Length of componentVersionString is 1 byte, allocate memory for it and
	 * append 1 bytes for null termination so that it can be used as a
	 * Null-terminated string.
	 */
 ...
 *
 * @param[in] data - pointer to downstream device parameter table raw data
 * @param[out] entry - pointer to downstream device parameter table entry
 * @param[out] versions - pointer to active/pending component version string
 *
 * @return 0 on success, otherwise -EINVAL if the input parameters' memory
 *         are not allocated, -EOVERFLOW if the payload length is not enough
 *         to decode the entry.
 ...
 * @param[out] active - pointer to active component version string
 * @param[out] pending - pointer to pending component version string
 *
