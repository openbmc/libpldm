...
/**
 * @brief Decode the response message for Querying Downstream Identifiers.
 *
 * @param[in] msg The PLDM message to decode.
 * @param[in] payload_length The length of the message payload.
 * @param[in,out] resp_data Pointer to the structure to store the decoded response data.
 * @param[in,out] downstream_devices Pointer to the variable field structure to store
 *                           the decoded downstream devices.
 * @return pldm_completion_codes
 *
 * @note Caller is responsible for memory alloc and dealloc of param
 *        'resp_data' and 'downstream_devices'
 */
