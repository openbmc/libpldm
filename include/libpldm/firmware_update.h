 ...
 * variable-length payload. The shared pldm_get_fd_data_* types are reused
 * by GetMetaData, which has an identical wire format.
 */

#define PLDM_GET_FD_DATA_REQ_BYTES	  5
#define PLDM_GET_FD_DATA_RESP_FIXED_BYTES 6

/** @struct pldm_get_fd_data_req
 *
 *  Decoded GetPackageData/GetMetaData request.
 */
struct pldm_get_fd_data_req {
	uint32_t data_transfer_handle;
	uint8_t transfer_operation_flag;
...
/** @struct pldm_get_fd_data_resp_fixed
 *
 *  Fixed-length header of a GetPackageData/GetMetaData response. The data
 *  portion is passed separately as a variable_field by the caller.
 */
struct pldm_get_fd_data_resp_fixed {
	uint8_t completion_code;
	uint32_t next_data_transfer_handle;
 ...
 *
 *  @param[in]  instance_id    - Message's instance id.
 *  @param[in]  resp_fixed     - Fixed-length response header.
 *  @param[in]  data           - Variable-length package data portion. May
 *                               be empty (length 0); ptr must be non-NULL
 *                               unless length is 0.
 *  @param[out] msg            - Response message.
 *  @param[in]  payload_length - Caller-allocated payload buffer size. Must
 *                               hold PLDM_GET_FD_DATA_RESP_FIXED_BYTES plus
 *                               data->length.
 *  @return 0 on success, a negative errno value on failure.
 */
