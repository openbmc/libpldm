 ...
 *
 *  hdr.length is encoded as supplied. The caller owns its value, which must
 *  account for the appended possible_states[] entries in addition to the
 *  fixed fields.
 *
 ...
 *  @return 0 on success, otherwise a negative errno value:
 *          -EINVAL - @p pdr, @p data or @p data_len is NULL
 ...
 *
 * @retval  0          - Success.
 * @retval -EINVAL     - Invalid input pointers, or possible_states->length
 *                       does not equal states->possible_states_size.
 ...
 *                    to the number of populated entries of .op_fields.
 *
 *  @return error code: 0 on success
 *                      -EINVAL if the function input parameters are incorrect
 *                      -EPROTO if the input request message is invalid
